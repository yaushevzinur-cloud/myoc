...
function header(title,sub=""){return `<div class="top"><div><span class="eyebrow">MYOS · V0.23.0</span><h1>${title}</h1><div class="muted">${sub}</div><span id="cloudSync" class="cloudSync">${window.MYOS_SYNC_STATUS||"☁ Проверка…"}</span></div><button class="mode" id="mode">${state.mode==="Вахта"?"⛺":"🏠"} ${state.mode}</button></div>`}
...
function contractDetail(id){
 ...
 shell(`<section class="screen workSectionScreen"><div class="screenTop"><button id="backContracts" class="backBtn">← Договоры</button><div><small>MYOS · V0.23.0</small><h2>№${c.number}</h2></div></div>
...
function nutrition(){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backMe" class="backBtn">← Я</button><div><small>MYOS · V0.23.0</small><h2>🍽 Питание</h2></div></div>
 ...
 <div class="nutritionActions"><button class="workPrimary" id="addMeal">＋ Добавить приём пищи</button><button class="workSecondary" id="importFood">✨ Импорт из ChatGPT</button></div>
 ...
 document.getElementById('addMeal').onclick=()=>mealAddChoice(date);document.getElementById('importFood').onclick=()=>nutritionImport(date);document.getElementById('foodTargets').onclick=()=>nutritionTargets();
...
function mealAddChoice(date){
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backNutrition" class="backBtn">← Питание</button><div><small>MYOS · V0.23.0</small><h2>Добавить еду</h2></div></div>
 <p class="sectionLead">Выбери самый удобный способ. Фото и описание можно проверить перед сохранением.</p>
 <section class="foodAddModes">
  <button class="card foodModeCard" id="foodByPhoto"><span>📷</span><div><b>По фотографии</b><small>AI распознает продукты, примерно оценит порции и КБЖУ.</small></div><i>›</i></button>
  <button class="card foodModeCard" id="foodByText"><span>💬</span><div><b>Описать словами</b><small>Например: 3 яйца, хлеб и чай с вареньем. AI подготовит оценку.</small></div><i>›</i></button>
  <button class="card foodModeCard" id="foodManual"><span>✍️</span><div><b>Ввести вручную</b><small>Обычная форма с готовыми КБЖУ.</small></div><i>›</i></button>
 </section></section>`);
 document.getElementById('backNutrition').onclick=()=>nutrition();
 document.getElementById('foodByPhoto').onclick=()=>aiMealForm(date,'photo');
 document.getElementById('foodByText').onclick=()=>aiMealForm(date,'text');
...
function aiMealForm(date,mode){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backAddMeal" class="backBtn">← Добавить</button><div><small>MYOS · V0.23.0</small><h2>${photo?'📷 Еда по фото':'💬 Еда по описанию'}</h2></div></div>
...
function normalizeImportedMeals(obj){
 let arr=Array.isArray(obj)?obj:(obj.meals||obj.items||obj.food||[]); if(!Array.isArray(arr))return [];
 return arr.map((x,i)=>({id:'imp-'+Date.now()+'-'+i,type:x.type||x.meal||x.category||'Импорт',name:x.name||x.title||x.food||`Приём пищи ${i+1}`,details:x.details||x.description||x.amount||'',kcal:+(x.kcal??x.calories??0)||0,protein:+(x.protein??x.p??0)||0,fat:+(x.fat??x.f??0)||0,carbs:+(x.carbs??x.c??0)||0}));
}
function nutritionImport(date){
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backNutrition" class="backBtn">← Питание</button><div><small>MYOS · V0.23.0</small><h2>✨ Импорт из ChatGPT</h2></div></div><section class="card importFoodCard"><b>Вставь JSON, который я подготовлю в чате</b><small>MyOS добавит все приёмы пищи и КБЖУ за выбранный день. Уже существующие записи не удаляются.</small><textarea id="foodImportText" placeholder='{"meals":[{"type":"Завтрак","name":"...","kcal":500,"protein":30,"fat":15,"carbs":60}]}'></textarea><div id="importPreview" class="importPreview"></div></section><button class="workPrimary" id="doFoodImport">Импортировать</button></section>`);
 document.getElementById('backNutrition').onclick=()=>nutrition();document.getElementById('doFoodImport').onclick=()=>{let obj;try{obj=JSON.parse(document.getElementById('foodImportText').value.trim())}catch(e){return alert('Не удалось прочитать данные. Вставьте JSON из ChatGPT целиком.')}const meals=normalizeImportedMeals(obj);if(!meals.length)return alert('В данных не найдено приёмов пищи.');nutritionDay(date).meals.push(...meals);save();alert(`Добавлено записей: ${meals.length}`);nutrition()}
}
function nutritionTargets(){ensureNutrition();const g=state.nutrition.targets;shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backNutrition" class="backBtn">← Питание</button><div><small>MYOS · V0.23.0</small><h2>⚙️ Дневные цели</h2></div></div><section class="card foodForm"><div class="foodMacroInputs"><label>Ккал<input id="tgK" type="number" value="${g.kcal}"></label><label>Белки, г<input id="tgP" type="number" value="${g.protein}"></label><label>Жиры, г<input id="tgF" type="number" value="${g.fat}"></label><label>Углеводы, г<input id="tgC" type="number" value="${g.carbs}"></label></div></section><button class="workPrimary" id="saveTargets">Сохранить цели</button></section>`);document.getElementById('backNutrition').onclick=()=>nutrition();document.getElementById('saveTargets').onclick=()=>{state.nutrition.targets={kcal:+tgK.value||0,protein:+tgP.value||0,fat:+tgF.value||0,carbs:+tgC.value||0};save();nutrition()}}
