...
function header(title,sub=""){return `<div class="top"><div><span class="eyebrow">MYOS · V0.24.6</span><h1>${title}</h1><div class="muted">${sub}</div><span id="cloudSync" class="cloudSync">${window.MYOS_SYNC_STATUS||"☁ Проверка…"}</span></div><button class="mode" id="mode">${state.mode==="Вахта"?"⛺":"🏠"} ${state.mode}</button></div>`}
...
const GYM_LEGACY_WORKOUT={id:"gym-legacy-2026-03-31",date:"2026-03-31",startedAt:null,finishedAt:null,location:"gym",type:"gym",source:"confirmed-legacy",migrated:true,exercises:[
 {id:"gym-entry-2026-03-31-run",exerciseId:"gym-ex-run",name:"Бег",category:"кардио",kind:"cardio",sets:[{id:"gym-set-2026-03-31-run-1",distanceKm:4,durationSeconds:1380}]},
 {id:"gym-entry-2026-03-31-neck-front",exerciseId:"gym-ex-neck-front",name:"Шея вперёд",category:"шея",kind:"strength",sets:[[30,15],[35,12],[40,10]].map((x,i)=>({id:`gym-set-2026-03-31-neck-front-${i+1}`,weightKg:x[0],reps:x[1]}))},
 {id:"gym-entry-2026-03-31-neck-back",exerciseId:"gym-ex-neck-back",name:"Шея назад",category:"шея",kind:"strength",sets:[],note:"Выполнение подтверждено; вес и повторения в исходной истории не указаны."},
 {id:"gym-entry-2026-03-31-neck-sides",exerciseId:"gym-ex-neck-sides",name:"Шея вправо/влево в тренажёре",category:"шея",kind:"strength",sets:[[25,25],[35,20],[40,16],[45,16]].map((x,i)=>({id:`gym-set-2026-03-31-neck-sides-${i+1}`,weightKg:x[0],reps:x[1]}))}
]};
...
function importLegacyGymData(){const g=state.fitness.gym;addUniqueById(g.exercises,GYM_LEGACY_EXERCISES);addUniqueById(g.workouts,[GYM_LEGACY_WORKOUT]);addUniqueById(g.legacyDrafts,GYM_LEGACY_DRAFTS)}
...
function contractDetail(id){
 ...
 shell(`<section class="screen workSectionScreen"><div class="screenTop"><button id="backContracts" class="backBtn">← Договоры</button><div><small>MYOS · V0.24.6</small><h2>№${c.number}</h2></div></div>
...
function nutrition(){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backMe" class="backBtn">← Я</button><div><small>MYOS · V0.24.6</small><h2>🍽 Питание</h2></div></div>
...
function mealAddChoice(date){
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backNutrition" class="backBtn">← Питание</button><div><small>MYOS · V0.24.6</small><h2>Добавить еду</h2></div></div>
...
function aiMealForm(date,mode){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backAddMeal" class="backBtn">← Добавить</button><div><small>MYOS · V0.24.6</small><h2>${photo?'📷 Еда по фото':'💬 Еда по описанию'}</h2></div></div>
