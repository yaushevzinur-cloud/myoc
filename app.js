...
// Existing Kegel practice started outside MyOS. Day 6 is a conservative
// continuation point; the owner can change it without fabricating completions.
...
if(!Number.isFinite(+state.fitness.kegel.startDay)||+state.fitness.kegel.startDay<1) state.fitness.kegel.startDay=6;
if(!Array.isArray(state.fitness.kegel.history)) state.fitness.kegel.history=[];
...
// Keep this pre-migration snapshot in memory and re-merge it until V0.24.1's
...
function header(title,sub=""){return `<div class="top"><div><span class="eyebrow">MYOS · V0.24.2</span><h1>${title}</h1><div class="muted">${sub}</div><span id="cloudSync" class="cloudSync">${window.MYOS_SYNC_STATUS||"☁ Проверка…"}</span></div><button class="mode" id="mode">${state.mode==="Вахта"?"⛺":"🏠"} ${state.mode}</button></div>`}
...
function contractDetail(id){
 ...
 shell(`<section class="screen workSectionScreen"><div class="screenTop"><button id="backContracts" class="backBtn">← Договоры</button><div><small>MYOS · V0.24.2</small><h2>№${c.number}</h2></div></div>
...
// Kegel progression advances only from recorded completions. Missed calendar
// days never change the suggested program day.
let kegelTimer=null,kegelRuntime=null;
function kegelHistory(){return state.fitness.kegel.history}
function nextKegelDay(){
 const completed=kegelHistory().filter(x=>x&&x.completedAt&&Number.isFinite(+x.programDay)).map(x=>+x.programDay);
 return Math.max(+state.fitness.kegel.startDay||1,completed.length?Math.max(...completed)+1:1);
}
function kegelPlan(day){
 const level=Math.min(3,Math.max(0,Math.floor((day-1)/5))),hold=[3,4,5,6][level],reps=[6,7,8,8][level],quick=[5,6,8,10][level];
 const phases=[{kind:"prepare",label:"Подготовка",seconds:20,instruction:"Сядь или ляг удобно. Дыши спокойно, живот и ягодицы мягкие."}];
 for(let i=0;i<reps;i++){phases.push({kind:"hold",label:`Сокращение ${i+1}/${reps}`,seconds:hold,instruction:"Мягко подтяни мышцы тазового дна. Не задерживай дыхание и не напрягай ягодицы."},{kind:"rest",label:"Полное расслабление",seconds:hold+2,instruction:"Полностью отпусти мышцы. Не начинай следующее сокращение, пока напряжение не ушло."})}
 for(let i=0;i<quick;i++){phases.push({kind:"quick",label:`Короткое ${i+1}/${quick}`,seconds:1,instruction:"Коротко сократи и сразу отпусти."},{kind:"rest",label:"Отпустить",seconds:2,instruction:"Полностью расслабь тазовое дно."})}
 phases.push({kind:"reverse",label:"Обратный Кегель",seconds:30+level*5,instruction:"На спокойном вдохе мягко расширь и отпусти область тазового дна, словно начинаешь мочеиспускание. Не тужься и не дави. Если не чувствуешь движение — просто дыши и расслабляйся."});
 return {day,hold,reps,quick,phases,total:phases.reduce((n,x)=>n+x.seconds,0)};
}
function kegelDateLabel(date){return new Date(date+"T12:00:00").toLocaleDateString("ru-RU",{day:"numeric",month:"short",year:"numeric"})}
function fitness(){
 const day=nextKegelDay(),plan=kegelPlan(day),history=kegelHistory().slice().sort((a,b)=>String(b.date).localeCompare(String(a.date)));
 shell(header("🏋️ Фитнес и тело","Безопасная последовательная практика")+`<section class="kegelHero card"><span class="kicker">СЛЕДУЮЩАЯ ТРЕНИРОВКА</span><h2>Кегель · день ${day}</h2><p>${plan.reps} контролируемых сокращений по ${plan.hold} сек. · ${plan.quick} коротких · мягкий reverse Kegel</p><small>Программа продолжится только после фактически выполненной тренировки.</small><button class="workPrimary" id="startKegel">Начать · ~${Math.ceil(plan.total/60)} мин</button></section>
 <section class="kegelSafety card"><b>Без боли и натуживания</b><small>Дыши свободно. При боли, онемении или выраженном дискомфорте остановись; при симптомах обратись к врачу или физиотерапевту тазового дна.</small></section>
 <div class="sectionTitle"><h2>История</h2><button class="kegelTextBtn" id="kegelSettings">Настроить / импортировать</button></div><section class="kegelHistory">${history.length?history.map(x=>`<article class="card"><span>✓</span><div><b>День ${x.programDay}</b><small>${kegelDateLabel(x.date)}${x.source==="manual"?" · внесено вручную":""}</small></div></article>`).join(""):`<article class="card emptyKegel"><b>История пока не перенесена</b><small>Укажи прежние выполненные дни через «Настроить / импортировать».</small></article>`}</section>`);
 bindMode();document.getElementById("startKegel").onclick=()=>startKegel(day);document.getElementById("kegelSettings").onclick=kegelSettings;
}
function stopKegelTimer(){if(kegelTimer){clearInterval(kegelTimer);kegelTimer=null}}
function startKegel(day){stopKegelTimer();kegelRuntime={plan:kegelPlan(day),phase:0,left:kegelPlan(day).phases[0].seconds,running:false};renderKegelTimer()}
function renderKegelTimer(){
 const r=kegelRuntime,p=r.plan.phases[r.phase],pct=Math.round((r.phase/r.plan.phases.length)*100);
 shell(`<section class="screen kegelTimerScreen"><div class="screenTop"><button id="exitKegel" class="backBtn">← Выйти</button><div><small>MYOS · V0.24.2</small><h2>Кегель · день ${r.plan.day}</h2></div></div><div class="kegelTimerCard card ${p.kind}"><span class="kicker">${p.label}</span><strong id="kegelSeconds">${r.left}</strong><p>${p.instruction}</p><div class="kegelPhaseBar"><i style="width:${pct}%"></i></div><small>Этап ${r.phase+1} из ${r.plan.phases.length}</small></div><button class="workPrimary" id="toggleKegel">${r.running?"Пауза":"Старт"}</button><button class="workSecondary kegelSkip" id="skipKegel">Следующий этап</button></section>`);
 document.getElementById("exitKegel").onclick=()=>{stopKegelTimer();r.running=false;if(confirm("Выйти? Незавершённая тренировка не будет записана."))fitness();else renderKegelTimer()};
 document.getElementById("toggleKegel").onclick=()=>{r.running=!r.running;stopKegelTimer();if(r.running)kegelTimer=setInterval(kegelTick,1000);renderKegelTimer()};
 document.getElementById("skipKegel").onclick=()=>advanceKegel();
}
function kegelTick(){if(!kegelRuntime||!kegelRuntime.running)return;kegelRuntime.left--;const el=document.getElementById("kegelSeconds");if(el)el.textContent=kegelRuntime.left;if(kegelRuntime.left<=0)advanceKegel()}
function advanceKegel(){
 stopKegelTimer();const r=kegelRuntime;r.phase++;
 if(r.phase>=r.plan.phases.length)return completeKegel(r.plan.day);
 r.left=r.plan.phases[r.phase].seconds;r.running=true;kegelTimer=setInterval(kegelTick,1000);renderKegelTimer();
}
function completeKegel(day){
 const date=keyToday(),exists=kegelHistory().some(x=>x.programDay===day&&x.completedAt);
 if(!exists)kegelHistory().push({id:`kegel-${day}-${date}`,programDay:day,date,completedAt:new Date().toISOString(),source:"timer"});
 save();kegelRuntime=null;shell(header("Тренировка завершена","Прогресс сохранён на этом устройстве и синхронизируется")+`<section class="kegelDone card"><strong>✓</strong><h2>День ${day} выполнен</h2><p>Следующая тренировка — день ${nextKegelDay()}. Нагрузка вырастет только постепенно.</p><button class="workPrimary" id="backFitness">К тренировкам</button></section>`);bindMode();document.getElementById("backFitness").onclick=fitness;
}
function kegelSettings(){
 const next=nextKegelDay();shell(`<section class="screen"><div class="screenTop"><button id="backFitness" class="backBtn">← Фитнес</button><div><small>MYOS · V0.24.2</small><h2>Прогресс Кегеля</h2></div></div><section class="card kegelForm"><h3>Точка продолжения</h3><p>Укажи день, который MyOS должен предложить, пока в истории нет более поздней выполненной тренировки.</p><label>Текущий / следующий день программы<input id="kegelStartDay" type="number" min="1" max="365" value="${state.fitness.kegel.startDay}"></label><button class="workPrimary" id="saveKegelLevel">Сохранить</button></section><section class="card kegelForm"><h3>Добавить прежнюю тренировку</h3><p>Запись считается реально выполненной и определяет следующий день программы.</p><label>Дата<input id="pastKegelDate" type="date" max="${keyToday()}" value="${keyToday()}"></label><label>День программы<input id="pastKegelDay" type="number" min="1" max="365" value="${Math.max(1,next-1)}"></label><button class="workPrimary" id="importKegel">Добавить в историю</button></section>`);
 document.getElementById("backFitness").onclick=fitness;
 document.getElementById("saveKegelLevel").onclick=()=>{state.fitness.kegel.startDay=Math.max(1,Math.floor(+document.getElementById("kegelStartDay").value||1));save();fitness()};
 document.getElementById("importKegel").onclick=()=>{const date=document.getElementById("pastKegelDate").value,day=Math.max(1,Math.floor(+document.getElementById("pastKegelDay").value||0));if(!date||date>keyToday())return alert("Выберите сегодняшнюю или прошедшую дату.");if(kegelHistory().some(x=>x.programDay===day&&x.completedAt))return alert("Этот день программы уже есть в истории.");kegelHistory().push({id:`kegel-manual-${day}-${date}`,programDay:day,date,completedAt:new Date(date+"T12:00:00").toISOString(),source:"manual"});save();fitness()};
}
...
function render(p){stopKegelTimer();current=p;document.querySelectorAll("nav button").forEach(b=>b.classList.toggle("active",b.dataset.page===p));({today,plan,add,progress,me,goals,languages,professional,work,nutrition,journal,fitness}[p]||today)();scrollTo(0,0)}
...
function nutrition(){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backMe" class="backBtn">← Я</button><div><small>MYOS · V0.24.2</small><h2>🍽 Питание</h2></div></div>
...
function mealAddChoice(date){
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backNutrition" class="backBtn">← Питание</button><div><small>MYOS · V0.24.2</small><h2>Добавить еду</h2></div></div>
...
function aiMealForm(date,mode){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backAddMeal" class="backBtn">← Добавить</button><div><small>MYOS · V0.24.2</small><h2>${photo?'📷 Еда по фото':'💬 Еда по описанию'}</h2></div></div>
