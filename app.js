...
function header(title,sub=""){return `<div class="top"><div><span class="eyebrow">MYOS · V0.24.5</span><h1>${title}</h1><div class="muted">${sub}</div><span id="cloudSync" class="cloudSync">${window.MYOS_SYNC_STATUS||"☁ Проверка…"}</span></div><button class="mode" id="mode">${state.mode==="Вахта"?"⛺":"🏠"} ${state.mode}</button></div>`}
...
const KEGEL_LEVELS=[
 ...
 {name:"Выносливость без перенапряжения",hold:8,relax:10,reps:10,fast:14,long:12,longReps:5,reverseCycles:4,reverse:12,rest:45,intensity:"~70%",intensityName:"Сильное контролируемое"}
];
...
function kegelPlan(level=state.fitness.kegel.selectedLevel){
 ...
 const add=(x,phase,action,seconds,repeat,total,instruction,intensity=l.intensity,intensityName=l.intensityName)=>phases.push({exercise:x.name,exerciseIndex:x.index,exerciseTotal:4,phase,action,seconds,repeat,total,instruction,intensity,intensityName,cycles:phase==='СЖАТЬ'?1:0});
 ...
 phases.push({exercise:'Отдых',exerciseIndex:1,exerciseTotal:4,phase:'ОТДЫХ',action:'ОТДЫХ',seconds:l.rest,nextExercise:quick,cycles:0,instruction:'Дыши спокойно. Можно начать раньше.',intensity:null});
 ...
 phases.push({exercise:'Отдых',exerciseIndex:2,exerciseTotal:4,phase:'ОТДЫХ',action:'ОТДЫХ',seconds:l.rest,nextExercise:long,cycles:0,instruction:'Полностью расслабься перед новым блоком.',intensity:null});
 ...
 const reverse=addExercise('Reverse Kegel','Освоить ощущение отпускания без натуживания',l.reverseCycles,l.reverse,null,null,'Спокойный вдох животом. Сознательно отпусти тазовое дно. Не тужься.');
 phases.push({exercise:'Отдых',exerciseIndex:3,exerciseTotal:4,phase:'ОТДЫХ',action:'ОТДЫХ',seconds:l.rest,nextExercise:reverse,cycles:0,instruction:'Подготовь спокойное дыхание и не сжимай мышцы.',intensity:null});
...
function contractDetail(id){
 ...
 shell(`<section class="screen workSectionScreen"><div class="screenTop"><button id="backContracts" class="backBtn">← Договоры</button><div><small>MYOS · V0.24.5</small><h2>№${c.number}</h2></div></div>
...
function nutrition(){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backMe" class="backBtn">← Я</button><div><small>MYOS · V0.24.5</small><h2>🍽 Питание</h2></div></div>
...
function mealAddChoice(date){
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backNutrition" class="backBtn">← Питание</button><div><small>MYOS · V0.24.5</small><h2>Добавить еду</h2></div></div>
...
function aiMealForm(date,mode){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backAddMeal" class="backBtn">← Добавить</button><div><small>MYOS · V0.24.5</small><h2>${photo?'📷 Еда по фото':'💬 Еда по описанию'}</h2></div></div>
