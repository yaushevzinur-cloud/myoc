...
function header(title,sub=""){return `<div class="top"><div><span class="eyebrow">MYOS · V0.24.8</span><h1>${title}</h1><div class="muted">${sub}</div><span id="cloudSync" class="cloudSync">${window.MYOS_SYNC_STATUS||"☁ Проверка…"}</span></div><button class="mode" id="mode">${state.mode==="Вахта"?"⛺":"🏠"} ${state.mode}</button></div>`}
...
function ensureFitness(){
 ...
 if(!Array.isArray(k.history))k.history=legacy?legacy.slice():[];
 const legacyGym=Array.isArray(state.fitness.gym)?state.fitness.gym:null;
 if(!state.fitness.gym||typeof state.fitness.gym!=="object"||Array.isArray(state.fitness.gym))state.fitness.gym={};
 const g=state.fitness.gym;
 if(!Array.isArray(g.workouts))g.workouts=legacyGym?legacyGym.slice():[];
 if(!Array.isArray(g.exercises))g.exercises=[];
 if(!Array.isArray(g.legacyDrafts))g.legacyDrafts=[];
 importLegacyGymData();
 importLegacyKegelRecords(KEGEL_LEGACY_HISTORY);
 if(!Number.isFinite(+k.programLevel)){k.programLevel=1;k.selectedLevel=1}
 else if(!Number.isFinite(+k.selectedLevel))k.selectedLevel=k.programLevel;
}
const GYM_CATEGORIES=["грудь","спина","ноги","плечи","бицепс","трицепс","шея","пресс/core","кардио","другое"];
const GYM_LEGACY_EXERCISES=[
 {id:"gym-ex-run",name:"Бег",category:"кардио",kind:"cardio"},
 {id:"gym-ex-neck-front",name:"Шея вперёд",category:"шея",kind:"strength"},
 {id:"gym-ex-neck-back",name:"Шея назад",category:"шея",kind:"strength"},
 {id:"gym-ex-neck-sides",name:"Шея вправо/влево в тренажёре",category:"шея",kind:"strength"},
 {id:"gym-ex-squat",name:"Присед со штангой",category:"ноги",kind:"strength"},
 {id:"gym-ex-calf",name:"Икры",category:"ноги",kind:"strength"},
 {id:"gym-ex-preacher",name:"Бицепс на скамье Скотта",category:"бицепс",kind:"strength"},
 {id:"gym-ex-abs-machine",name:"Пресс в тренажёре",category:"пресс/core",kind:"strength"},
 {id:"gym-ex-biceps-crossover",name:"Бицепс в кроссовере",category:"бицепс",kind:"strength"},
 {id:"gym-ex-triceps-crossover",name:"Трицепс в кроссовере",category:"трицепс",kind:"strength"},
 {id:"gym-ex-leg-extension",name:"Разгибание ног в тренажёре",category:"ноги",kind:"strength"},
 {id:"gym-ex-triceps-pushdown",name:"Разгибание рук на блоке",category:"трицепс",kind:"strength"},
 {id:"gym-ex-cindy",name:"Cindy",category:"другое",kind:"circuit"},
 {id:"gym-ex-plank",name:"Планка",category:"пресс/core",kind:"strength"},
 {id:"gym-ex-french-press",name:"Французский жим гантелями",category:"трицепс",kind:"strength"},
 {id:"gym-ex-hammer",name:"Молотковые сгибания",category:"бицепс",kind:"strength"},
 {id:"gym-ex-biceps-machine",name:"Тяга / упражнение на бицепс в тренажёре",category:"бицепс",kind:"strength"},
 {id:"gym-ex-biceps-triceps",name:"Бицепс + трицепс на тренажёре",category:"другое",kind:"strength"},
 {id:"gym-ex-barbell-squat",name:"Присед со штангой",category:"ноги",kind:"strength"}
];
const legacyStrength=(date,key,exerciseId,name,category,sets,note)=>({id:`gym-entry-${date}-${key}`,exerciseId,name,category,kind:"strength",sets:sets.map((x,i)=>({id:`gym-set-${date}-${key}-${i+1}`,weightKg:x[0],reps:x[1],...(x[2]?{note:x[2]}:{})})),...(note?{note}:{})});
const legacyCardio=(date,key,distanceKm,durationSeconds,note)=>({id:`gym-entry-${date}-${key}`,exerciseId:"gym-ex-run",name:"Бег",category:"кардио",kind:"cardio",sets:[{id:`gym-set-${date}-${key}-1`,distanceKm,durationSeconds,...(note?{note}:{})}]});
const GYM_LEGACY_WORKOUTS=[
 {id:"gym-legacy-2026-01-08",date:"2026-01-08",startedAt:null,finishedAt:null,location:"gym",type:"gym",status:"finished",source:"confirmed-legacy",migrated:true,exercises:[
  legacyStrength("2026-01-08","neck-sides","gym-ex-neck-sides","Шея вправо/влево в тренажёре","шея",[[30,20],[35,15]]),
  legacyStrength("2026-01-08","triceps-pushdown","gym-ex-triceps-pushdown","Разгибание рук на блоке","трицепс",[[35,20],[40,18]])
 ]},
 {id:"gym-legacy-2026-01-19",date:"2026-01-19",startedAt:null,finishedAt:null,location:"gym",type:"gym",status:"finished",source:"confirmed-legacy",migrated:true,exercises:[
  legacyCardio("2026-01-19","run",5,null,"Дистанция подтверждена; время не указано."),
  legacyStrength("2026-01-19","neck-back","gym-ex-neck-back","Шея назад","шея",[[35,20],[40,18],[45,15],[50,13]]),
  legacyStrength("2026-01-19","neck-front","gym-ex-neck-front","Шея вперёд","шея",[[20,20],[30,12],[35,8]]),
  legacyStrength("2026-01-19","squat","gym-ex-squat","Присед со штангой","ноги",[[0,12,"Вес первого подхода не указан."],[20,10,"В исходной записи: + зелёные по 10 кг; полный вес не подтверждён."],[120,10]])
 ]},
 {id:"gym-legacy-2026-01-23",date:"2026-01-23",startedAt:null,finishedAt:null,location:"gym",type:"gym",status:"finished",source:"confirmed-legacy",migrated:true,exercises:[
  legacyCardio("2026-01-23","run",5,1680),
  legacyStrength("2026-01-23","preacher","gym-ex-preacher","Бицепс на скамье Скотта","бицепс",[[20,10],[15,16,"В исходной записи: 2×15 кг."],[17.5,10],[15,10]]),
  legacyStrength("2026-01-23","abs-machine","gym-ex-abs-machine","Пресс в тренажёре","пресс/core",[[20,20],[20,20],[20,20]])
 ]},
 {id:"gym-legacy-2026-03-05",date:"2026-03-05",startedAt:null,finishedAt:null,location:"gym",type:"gym",status:"finished",source:"confirmed-legacy",migrated:true,exercises:[
  legacyCardio("2026-03-05","run",3,1200),
  legacyStrength("2026-03-05","neck-back","gym-ex-neck-back","Шея назад","шея",[[20,25],[30,20]]),
  legacyStrength("2026-03-05","neck-front","gym-ex-neck-front","Шея вперёд","шея",[[20,20],[25,15],[20,20]]),
  legacyStrength("2026-03-05","squat","gym-ex-squat","Присед со штангой","ноги",[[50,20],[90,15],[100,10]]),
  legacyStrength("2026-03-05","calf","gym-ex-calf","Икры","ноги",[[27.5,15],[27.5,20],[27.5,20]]),
  legacyStrength("2026-03-05","biceps-crossover","gym-ex-biceps-crossover","Бицепс в кроссовере","бицепс",[[30,20],[35,15]]),
  legacyStrength("2026-03-05","triceps-crossover","gym-ex-triceps-crossover","Трицепс в кроссовере","трицепс",[[25,25],[30,20],[35,15]])
 ]},
 {id:"gym-legacy-2026-03-07",date:"2026-03-07",startedAt:null,finishedAt:null,location:"gym",type:"gym",status:"finished",source:"confirmed-legacy",migrated:true,exercises:[
  legacyCardio("2026-03-07","run",4,1500),
  {id:"gym-entry-2026-03-07-neck-sides",exerciseId:"gym-ex-neck-sides",name:"Шея вправо/влево",category:"шея",kind:"strength",sets:[],note:"Выполнение подтверждено; вес и повторения не сохранились."}
 ]},
 {id:"gym-legacy-2026-03-21",date:"2026-03-21",startedAt:null,finishedAt:null,location:"gym",type:"gym",status:"finished",source:"confirmed-legacy",migrated:true,exercises:[
  legacyCardio("2026-03-21","run",3,1200),
  legacyStrength("2026-03-21","neck-front","gym-ex-neck-front","Шея вперёд","шея",[[25,20],[30,15],[35,10]]),
  legacyStrength("2026-03-21","neck-back","gym-ex-neck-back","Шея назад","шея",[[35,10]]),
  legacyStrength("2026-03-21","biceps-crossover","gym-ex-biceps-crossover","Бицепс в кроссовере","бицепс",[[30,20],[35,15],[40,13],[45,10]]),
  legacyStrength("2026-03-21","triceps-crossover","gym-ex-triceps-crossover","Трицепс снизу вверх","трицепс",[[30,20],[35,15]])
 ]},
 {id:"gym-legacy-2026-03-31",date:"2026-03-31",startedAt:null,finishedAt:null,location:"gym",type:"gym",status:"finished",source:"confirmed-legacy",migrated:true,exercises:[
  legacyCardio("2026-03-31","run",2,600),
  legacyStrength("2026-03-31","neck-front","gym-ex-neck-front","Шея вперёд","шея",[[30,15],[35,12],[40,10]]),
  legacyStrength("2026-03-31","neck-sides","gym-ex-neck-sides","Шея вправо/влево в тренажёре","шея",[[25,25],[35,20],[40,16],[45,16]]),
  legacyStrength("2026-03-31","biceps-machine","gym-ex-biceps-machine","Бицепс в тренажёре","бицепс",[[35,20],[45,16],[50,10]])
 ]},
 {id:"gym-legacy-2026-08-21",date:"2026-08-21",startedAt:null,finishedAt:null,location:"home",type:"circuit",status:"finished",source:"confirmed-legacy",migrated:true,exercises:[
  {id:"gym-entry-2026-08-21-cindy",exerciseId:"gym-ex-cindy",name:"Cindy",category:"другое",kind:"circuit",sets:[{id:"gym-set-2026-08-21-cindy-1",rounds:15,durationSeconds:1200,note:"AMRAP 20 мин · каждый круг: 5 подтягиваний + 10 отжиманий + 15 приседаний"}],note:"Cindy · AMRAP 20 минут: 5 подтягиваний, 10 отжиманий, 15 приседаний. Результат: 15 кругов."}
 ]},
 {id:"gym-legacy-2026-08-26",date:"2026-08-26",startedAt:null,finishedAt:null,location:"home",type:"circuit",status:"finished",source:"confirmed-legacy",migrated:true,exercises:[
  legacyStrength("2026-08-26","circuit","gym-ex-cindy","Круговой комплекс","другое",[[0,24,"24 круга: 5 отжиманий, 10 пресс, 15 приседаний за круг."]]),
  legacyStrength("2026-08-26","french","gym-ex-french-press","Французский жим гантелями","трицепс",[[0,50],[0,50],[0,50]],"3 круга; вес гантелей не указан."),
  legacyStrength("2026-08-26","hammer","gym-ex-hammer","Молотковые сгибания","бицепс",[[0,30],[0,30],[0,30]],"3 круга; вес гантелей не указан."),
  legacyStrength("2026-08-26","plank","gym-ex-plank","Планка","пресс/core",[[0,270,"4 мин 30 сек"]])
 ]},
 {id:"gym-legacy-2026-09-21",date:"2026-09-21",startedAt:null,finishedAt:null,location:"gym",type:"gym",status:"finished",source:"confirmed-legacy",migrated:true,exercises:[
  legacyStrength("2026-09-21","leg-extension","gym-ex-leg-extension","Разгибание ног в тренажёре","ноги",[[75,20],[90,15],[105,10]])
 ]},
 {id:"gym-legacy-2026-09-24",date:"2026-09-24",startedAt:null,finishedAt:null,location:"home",type:"circuit",status:"finished",source:"confirmed-legacy",migrated:true,exercises:[
  {id:"gym-entry-2026-09-24-cindy",exerciseId:"gym-ex-cindy",name:"Cindy",category:"другое",kind:"circuit",sets:[{id:"gym-set-2026-09-24-cindy-1",rounds:18,durationSeconds:1200,note:"AMRAP 20 мин · каждый круг: 5 подтягиваний + 10 отжиманий + 15 приседаний"}],note:"Cindy · AMRAP 20 минут: 5 подтягиваний, 10 отжиманий, 15 приседаний. Результат: 18 кругов."}
 ]}
];
const GYM_LEGACY_DRAFTS=[
 {id:"gym-draft-run-1km",exerciseId:"gym-ex-run",name:"Бег",kind:"cardio",sets:[{id:"gym-draft-set-run-1km",distanceKm:1,durationSeconds:360}],date:null,note:"Дата в исходной истории не подтверждена."},
 {id:"gym-draft-biceps-machine",exerciseId:"gym-ex-biceps-machine",name:"Тяга / упражнение на бицепс в тренажёре",kind:"strength",sets:[[35,20],[45,16],[50,10]].map((x,i)=>({id:`gym-draft-set-biceps-${i+1}`,weightKg:x[0],reps:x[1]})),date:null,note:"Дата в исходной истории не подтверждена."},
 {id:"gym-draft-biceps-triceps",exerciseId:"gym-ex-biceps-triceps",name:"Бицепс + трицепс на тренажёре",kind:"strength",sets:[25,20,15,12,10,8].map((reps,i)=>({id:`gym-draft-set-combo-${i+1}`,reps})),date:null,note:"Вес и дата в исходной истории не подтверждены."},
 {id:"gym-draft-squat",exerciseId:"gym-ex-barbell-squat",name:"Присед со штангой",kind:"strength",sets:[{id:"gym-draft-set-squat-1",weightKg:70,reps:20},{id:"gym-draft-set-squat-2",weightKg:90,reps:15,calculated:true,note:"Вычислено из 70 кг + 10 кг с каждой стороны."}],date:null,note:"После второго подхода добавлено ещё по 5 кг с каждой стороны; повторения и факт выполнения подхода не подтверждены."}
];
function addUniqueById(target,items){items.forEach(item=>{if(!target.some(x=>x&&x.id===item.id))target.push(clone(item))})}
function importLegacyGymData(){
 const g=state.fitness.gym;
 addUniqueById(g.exercises,GYM_LEGACY_EXERCISES);
 GYM_LEGACY_WORKOUTS.forEach(item=>{
  const existing=g.workouts.find(x=>x&&x.id===item.id);
  if(!existing)g.workouts.push(clone(item));
  else if(existing.source==="confirmed-legacy"&&existing.migrated)Object.assign(existing,clone(item));
 });
 addUniqueById(g.legacyDrafts,GYM_LEGACY_DRAFTS);
}
const KEGEL_LEGACY_HISTORY=[
 {date:"2026-09-08",note:"Начало ранее начатой в ChatGPT программы: контролируемое сокращение и полноценное расслабление."},
 {date:"2026-09-09",note:"Следующая последовательно выполненная тренировка программы."},
 {date:"2026-09-21",note:"Контролируемые и короткие сокращения, постепенные удержания; мягкое освоение reverse Kegel. Обычное сокращение и расслабление уже различаются, техника reverse Kegel пока требует внимания."}
];
function legacyKegelId(date){return `kegel-legacy-${date}-kegel`}
function importLegacyKegelRecords(records){
 if(!state.fitness||!state.fitness.kegel||!Array.isArray(state.fitness.kegel.history))return 0;
 let changed=0;
 (Array.isArray(records)?records:[]).forEach(input=>{
   const date=localDateKey(input&&input.date);if(!date)return;
   const id=legacyKegelId(date),history=state.fitness.kegel.history;
   let record=history.find(x=>x&&x.id===id);
   // An exact MyOS session for the date is stronger evidence than a placeholder.
   if(!record&&history.some(x=>x&&x.date===date&&x.completed&&x.source!=="ChatGPT legacy"))return;
   if(!record){record={id,date,workoutType:"kegel",completed:true,source:"ChatGPT legacy",legacy:true,imported:true};history.push(record);changed++}
   // A later journal import enriches the same stable entity; absent parameters
   // remain absent rather than becoming invented zeroes.
   Object.entries(input||{}).forEach(([key,value])=>{if(value!==undefined&&value!==null&&value!==""&&JSON.stringify(record[key])!==JSON.stringify(value)){record[key]=clone(value);changed++}});
   record.id=id;record.date=date;record.workoutType="kegel";record.completed=true;record.source="ChatGPT legacy";record.legacy=true;record.imported=true;
 });
 return changed;
}
ensureFitness();
const KEGEL_LEVELS=[
 {name:"Продолжение · техника",hold:5,relax:7,reps:8,fast:10,long:5,longReps:3,reverseCycles:3,reverse:10,rest:30,intensity:"~50%",intensityName:"Среднее сокращение"},
 {name:"Контроль и расслабление",hold:5,relax:7,reps:8,fast:10,long:8,longReps:5,reverseCycles:3,reverse:10,rest:40,intensity:"~50%",intensityName:"Среднее сокращение"},
 {name:"Уверенный контроль",hold:7,relax:8,reps:9,fast:12,long:10,longReps:5,reverseCycles:4,reverse:10,rest:45,intensity:"~60%",intensityName:"Сильное контролируемое"},
 {name:"Выносливость без перенапряжения",hold:8,relax:10,reps:10,fast:14,long:10,longReps:5,reverseCycles:4,reverse:12,rest:45,intensity:"~60%",intensityName:"Контролируемое"}
];
...
function suggestedKegelLevel(){ensureFitness();return Math.min(KEGEL_LEVELS.length-1,Math.max(1,Math.floor(kegelCompletedCount()/4)))}
function kegelPlan(level=state.fitness.kegel.selectedLevel){
 const index=Math.max(0,Math.min(KEGEL_LEVELS.length-1,+level||0)),l=KEGEL_LEVELS[index],phases=[],exercises=[];
 const addExercise=(name,goal,reps,work,relax,intensity,technique)=>{const x={index:exercises.length+1,name,goal,reps,work,relax,intensity,technique};exercises.push(x);return x};
 const add=(x,phase,action,seconds,repeat,total,instruction,intensity,intensityName)=>phases.push({exercise:x.name,exerciseIndex:x.index,exerciseTotal:6,phase,action,seconds,repeat,total,instruction,intensity:intensity===undefined?l.intensity:intensity,intensityName:intensityName===undefined?l.intensityName:intensityName,cycles:phase==='СЖАТЬ'?1:0});
 const breath=addExercise('Дыхание животом','Начать с мягкого расслабления',4,5,5,null,'Лёжа или сидя: вдох — живот свободно расширяется, выдох — без усилия. Не выталкивай мышцы вниз.');
 for(let i=1;i<=4;i++){add(breath,'ВДОХ','СПОКОЙНЫЙ ВДОХ',5,i,4,'Дыши животом и позволь тазовому дну расслабиться.',null,'Без усилия');add(breath,'ВЫДОХ','СПОКОЙНЫЙ ВЫДОХ',5,i,4,'Не сжимай и не тужься.',null,'Без усилия')}
 const control=addExercise('Контролируемые сокращения','Различать сокращение и полное расслабление',l.reps,l.hold,l.relax,l.intensity,'Не задерживай дыхание. На расслаблении полностью отпусти тазовое дно.');
 for(let i=1;i<=l.reps;i++){add(control,'СЖАТЬ','СЖАТЬ',l.hold,i,l.reps,'Сократи тазовое дно контролируемо.');add(control,'РАССЛАБИТЬ','РАССЛАБИТЬ ПОЛНОСТЬЮ',l.relax,i,l.reps,'0% · отпусти мышцы',null,'0% · полное расслабление')}
 const quick=addExercise('Быстрые сокращения','Точный ритм без угадывания',l.fast,1,1,'Мягко · без максимального усилия','Дыши свободно: одна секунда сокращения, одна секунда полного отпускания.');
 phases.push({exercise:'Отдых',exerciseIndex:control.index,exerciseTotal:6,phase:'ОТДЫХ',action:'ОТДЫХ',seconds:l.rest,nextExercise:quick,cycles:0,instruction:'Дыши спокойно. Можно начать раньше.',intensity:null});
 for(let i=1;i<=l.fast;i++){add(quick,'СЖАТЬ','СЖАТЬ',1,i,l.fast,'Коротко сократи.','Мягко','Мягкое сокращение');add(quick,'ОТПУСТИТЬ','ОТПУСТИТЬ',1,i,l.fast,'Полностью отпусти.',null,'0% · полное расслабление')}
 const long=addExercise('Длинные удержания','Удерживать контроль без задержки дыхания',l.longReps,l.long,10,l.intensity,'Сжимай контролируемо и спокойно дыши. Затем полностью расслабь мышцы.');
 phases.push({exercise:'Отдых',exerciseIndex:quick.index,exerciseTotal:6,phase:'ОТДЫХ',action:'ОТДЫХ',seconds:l.rest,nextExercise:long,cycles:0,instruction:'Полностью расслабься перед новым блоком.',intensity:null});
 for(let i=1;i<=l.longReps;i++){add(long,'СЖАТЬ','СЖАТЬ И УДЕРЖИВАТЬ',l.long,i,l.longReps,'Продолжай спокойно дышать.');add(long,'РАССЛАБИТЬ','РАССЛАБИТЬ ПОЛНОСТЬЮ',10,i,l.longReps,'0% · полностью отпусти',null,'0% · полное расслабление')}
 const reverse=addExercise('Мягкое отпускание (обратный Кегель)','Спокойный вдох без попытки выдавить мышцы вниз',l.reverseCycles,l.reverse,null,null,'Если отдельное ощущение не возникает — просто дыши животом. Не тужься и не выталкивай мышцы вниз.');
 phases.push({exercise:'Отдых',exerciseIndex:long.index,exerciseTotal:6,phase:'ОТДЫХ',action:'ОТДЫХ',seconds:l.rest,nextExercise:reverse,cycles:0,instruction:'Подготовь спокойное дыхание и не сжимай мышцы.',intensity:null});
 for(let i=1;i<=l.reverseCycles;i++)add(reverse,'REVERSE KEGEL','РАССЛАБЛЕНИЕ / REVERSE KEGEL',l.reverse,i,l.reverseCycles,'НЕ СЖИМАЙ. Спокойный вдох животом. Сознательно отпусти тазовое дно. Не тужься.',null,'Без силового усилия');
 const movement=kegelCompletedCount()%2===0
  ?addExercise('Ягодичный мостик с дыханием','Связать движение и дыхание, не перенапрягая тазовое дно',5,4,6,'Мягко','Лёжа на спине, согни колени. На выдохе плавно приподними таз, на вдохе опусти и полностью расслабься.')
  :addExercise('Встать и сесть с дыханием','Контроль тазового дна в обычном движении',5,4,6,'Мягко','Сядь на устойчивый стул. На выдохе встань с лёгким сокращением, на вдохе сядь и полностью расслабься.');
 phases.push({exercise:'Отдых',exerciseIndex:reverse.index,exerciseTotal:6,phase:'ОТДЫХ',action:'ОТДЫХ',seconds:l.rest,nextExercise:movement,cycles:0,instruction:'Перейди в положение для следующего упражнения.',intensity:null});
 for(let i=1;i<=5;i++){add(movement,'ДВИЖЕНИЕ',movement.name==='Ягодичный мостик с дыханием'?'ПОДНИМИ ТАЗ НА ВЫДОХЕ':'ВСТАНЬ НА ВЫДОХЕ',4,i,5,movement.name==='Ягодичный мостик с дыханием'?'Лёжа на спине с согнутыми коленями, подними таз на выдохе с лёгким сокращением.':'С устойчивого стула встань на выдохе с лёгким сокращением.','Мягко','Мягко');add(movement,'РАССЛАБИТЬ',movement.name==='Ягодичный мостик с дыханием'?'ОПУСТИ ТАЗ И ОТПУСТИ':'СЯДЬ И ОТПУСТИ',6,i,5,'На вдохе полностью отпусти тазовое дно.',null,'0% · расслабление')}
 return {level:index,name:l.name,number:kegelCompletedCount()+1,phases,exercises,duration:phases.reduce((n,p)=>n+p.seconds,0),cycles:phases.reduce((n,p)=>n+(p.cycles||0),0)};
}
function kegelPlanHTML(plan){return `<section class="kegelPlan"><header><b>Сегодня · ~${Math.ceil(plan.duration/60)} минут</b><span>Тренировка №${plan.number}</span></header>${plan.exercises.map(x=>`<article><i>${x.index}</i><div><b>${escapeHtml(x.name)}</b><small>${x.reps} ${x.reps===1?'цикл':'повторов'}${x.work?` · ${x.work} сек / отдых ${x.relax} сек`:''}${x.intensity?` · ${escapeHtml(x.intensity)}`:''}</small><p>${escapeHtml(x.goal)}</p></div></article>`).join('')}</section>`}
...
function kegelTodayCard(){const done=kegelToday(),plan=kegelPlan();return `<button id="todayKegel" class="todayKegel card ${done?'done':''}"><span>${done?'✓':'🧘'}</span><span><b>${done?'Кегель выполнен сегодня':'Кегель · сегодня'}</b><small>${done?(done.legacy?'Импортировано из ChatGPT':`${Math.ceil(done.duration/60)} мин · ${escapeHtml(done.levelName)}`):`~ ${Math.ceil(plan.duration/60)} мин`}</small></span><i>${done?'История':'Начать'}</i></button>`}
function today(){
...
function openTaskTimeDialog(taskId){
 const t=state.tasks.find(x=>x.id==taskId);if(!t)return;
 const old=document.getElementById("taskTimeModal");if(old)old.remove();
 const wrap=document.createElement("div");wrap.id="taskTimeModal";wrap.className="modalShade";
 wrap.innerHTML=`<section class="planModal card"><button class="modalClose" id="taskTimeCancel">×</button><span class="kicker">🕒 ТОЧНОЕ ВРЕМЯ</span><h2>${escapeHtml(t.title)}</h2><p class="muted">Время начала задачи. Длительность остаётся отдельным параметром.</p><label class="dateLabel">Начать в<input id="taskTimeInput" type="time" value="${t.planTime||''}"></label><div class="hActions"><button class="primary" id="taskTimeApply">Сохранить время</button>${t.planTime?'<button id="taskTimeClear">Убрать время</button>':''}</div></section>`;
 document.body.appendChild(wrap);taskTimeCancel.onclick=()=>wrap.remove();wrap.onclick=e=>{if(e.target===wrap)wrap.remove()};
 taskTimeApply.onclick=()=>{t.planTime=taskTimeInput.value||"";rememberPlan(t,t.planTime?"Назначено время "+t.planTime:"Точное время убрано");save();wrap.remove();plan()};
 const clear=document.getElementById("taskTimeClear");if(clear)clear.onclick=()=>{t.planTime="";rememberPlan(t,"Точное время убрано");save();wrap.remove();plan()};
}
function horizonCard(t){
 ...
 <small>${periodLabel(t)}${t.planDate&&t.horizon==="week"?" · день: "+shortDate(t.planDate):""}${t.planTime?" · 🕒 "+t.planTime:""} · ${t.priority||"Обычный"} · ${t.mins||30} мин</small>
...
function todayBoard(){
 ...
 return `${calendarStrip()}${overdueHTML}<section class="addTask card"><input id="taskTitle" placeholder="${isTodayDate(calDate)?"Что нужно сделать сегодня?":"Что нужно сделать "+shortDate(calDate)+"?"}">${areaPicker()}<div class="row2"><select id="taskPriority"><option>Высокий</option><option selected>Обычный</option><option>Низкий</option></select><select id="taskMins"><option value="15">15 мин</option><option value="30">30 мин</option><option value="60" selected>60 мин</option><option value="90">90 мин</option></select></div><label>Точное время <span class="muted">необязательно</span><input id="taskTime" type="time"></label><button class="primary" id="createTask">${selectedDayButtonLabel(calDate)}</button></section>
...
function boardTask(t){
 ...
 return `<article class="boardTask ${areaCls(t)}" draggable="true" data-taskid="${t.id}"><strong>${t.priority==="Высокий"?"🔥 ":""}${t.title}</strong>${areaTag(t)}<small>${t.planTime?"🕒 "+t.planTime+" · ":""}${t.priority||"Обычный"} · ${t.mins||30} мин${t.waitingFor?" · ждём: "+t.waitingFor:""}</small><div class="taskMoves">${moves.map(m=>`<button data-move="${t.id}:${m[0]}">${m[1]}</button>`).join("")}<button data-tasktime="${t.id}">${t.planTime?"🕒 "+t.planTime:"🕒 Время"}</button><button data-replan="${t.id}">📅 Перенести</button><button data-up="${t.id}:week">↑ В неделю</button><button data-up="${t.id}:month">↑ В месяц</button><button data-remove="${t.id}">Удалить</button></div></article>`;
}
...
function bindPlanner(){
 ...
 document.querySelectorAll("[data-replan]").forEach(b=>b.onclick=()=>openPlanDateDialog(+b.dataset.replan,"replan"));document.querySelectorAll("[data-tasktime]").forEach(b=>b.onclick=()=>openTaskTimeDialog(+b.dataset.tasktime));
...
function contractDetail(id){
 ...
 shell(`<section class="screen workSectionScreen"><div class="screenTop"><button id="backContracts" class="backBtn">← Договоры</button><div><small>MYOS · V0.24.8</small><h2>№${c.number}</h2></div></div>
...
function settingsScreen(){
 ...
 shell(`<section class="screen"><div class="screenTop"><button id="backSettings" class="backBtn">← Я</button><div><small>MYOS · V0.24.8</small><h2>⚙️ Настройки MyOS</h2></div></div><section class="settings">
 ...
 <article class="card"><b>Версия приложения</b><small>MyOS V0.24.8</small></article>
...
function fitness(){
 ensureFitness();clearInterval(workoutInterval);const plan=kegelPlan(),done=kegelToday(),history=state.fitness.kegel.history.slice().sort((a,b)=>String(b.date||b.completedAt).localeCompare(String(a.date||a.completedAt))),completed=kegelCompletedCount();
 shell(`<section class="screen fitnessScreen"><div class="screenTop"><button id="backFitness" class="backBtn">← Я</button><div><small>MYOS · V0.24.8</small><h2>🏋️ Фитнес / Тело</h2></div></div><p class="sectionLead">Центр тренировок и прогресса тела.</p><section class="fitnessDirections"><button id="openGym" class="card active"><span>🏋️</span><b>Тренажёрный зал</b><small>Дневник подходов и прогресса</small></button><article class="card"><span>🏠</span><b>Домашние тренировки</b><small>Интервальные программы — далее</small></article><button id="openKegel" class="card active"><span>🧘</span><b>Кегель / тазовое дно</b><small>Ежедневная спокойная практика</small></button><button id="openFitnessHistory" class="card active"><span>📊</span><b>История тренировок</b><small>Кегель · зал · будущие типы</small></button></section><section class="card kegelHero"><span class="kicker">КЕГЕЛЬ — ТРЕНИРОВКА СЕГОДНЯ</span><h2>${done?'✓ Выполнена сегодня':`Следующая тренировка · ${escapeHtml(plan.name)}`}</h2><p>${done&&done.legacy?'Выполнено до появления MyOS Player · неизвестные параметры не выдуманы':`Тренировка №${plan.number} · прогресс по фактически выполненным занятиям`}</p>${kegelPlanHTML(plan)}<p class="kegelSafety">Если возникает боль, тяжесть или усиливается напряжение в промежности, останови занятие. Дыши свободно; обратное отпускание не требует натуживания. При повторяющихся симптомах обсуди программу со специалистом по тазовому дну.</p><label>Программа<select id="kegelLevel">${KEGEL_LEVELS.map((x,i)=>`<option value="${i}" ${i===plan.level?'selected':''}>${i+1}. ${escapeHtml(x.name)}${i===suggestedKegelLevel()?' · рекомендован':''}</option>`).join('')}</select></label><button class="workPrimary" id="startKegel">${done?'Повторить тренировку':'Начать тренировку'}</button><button class="workSecondary" id="importKegel">Дополнить историю из ChatGPT</button></section><section class="kegelStats"><div class="card"><small>ТЕКУЩАЯ СЕРИЯ</small><b>${kegelStreak()} дней</b></div><div class="card"><small>ВСЕГО</small><b>${completed}</b></div></section><div class="sectionTitle"><h2>История Кегеля</h2><span>по датам</span></div><section class="kegelHistory">${history.length?history.map(x=>`<button class="card" data-kegel-history="${x.id}"><b>${x.source==='manual-import'?'Перенесённый прогресс':new Date((x.date||'')+'T12:00:00').toLocaleDateString('ru-RU',{day:'numeric',month:'long'})}</b><small>${x.source==='manual-import'?`${x.sessions} выполненных тренировок`:x.legacy?`Импортировано из ChatGPT · ${escapeHtml(x.note||'точные параметры не указаны')}`:`${escapeHtml(x.levelName)} · ${Math.ceil(x.duration/60)} мин · ${x.cycles} циклов`}</small><i>›</i></button>`).join(''):`<article class="card emptyFood"><b>История пока пуста</b><small>Завершённые тренировки появятся здесь и синхронизируются между устройствами.</small></article>`}</section></section>`);
 document.getElementById('backFitness').onclick=()=>render('me');document.getElementById('openGym').onclick=gym;document.getElementById('openFitnessHistory').onclick=fitnessHistory;document.getElementById('openKegel').onclick=()=>document.querySelector('.kegelHero').scrollIntoView({behavior:'smooth'});document.getElementById('kegelLevel').onchange=e=>{state.fitness.kegel.selectedLevel=+e.target.value;state.fitness.kegel.programLevel=+e.target.value;save();fitness()};document.getElementById('startKegel').onclick=()=>startKegel();document.getElementById('importKegel').onclick=()=>{const value=prompt('Вставьте JSON старого журнала: одну запись или массив записей с датой. Повторный импорт безопасно дополнит существующие даты.','');if(value===null||!value.trim())return;try{const parsed=JSON.parse(value),records=Array.isArray(parsed)?parsed:[parsed],changed=importLegacyKegelRecords(records);if(!changed)return alert('Новых или более точных данных не найдено.');save();fitness()}catch(e){alert('Не удалось прочитать JSON. Проверьте формат старого журнала.')}};document.querySelectorAll('[data-kegel-history]').forEach(b=>b.onclick=()=>kegelHistoryDetail(b.dataset.kegelHistory));
}

let gymView="today",activeGymExerciseId=null;
function gymRecordLabel(entry,set){if(entry.kind==='cardio')return `${set.distanceKm??'—'} км · ${formatTimer(set.durationSeconds||0)}`;if(entry.kind==='circuit')return `${set.rounds??set.reps??'—'} кругов · ${set.durationSeconds?Math.round(set.durationSeconds/60)+' мин':'время не указано'}`;return `${set.weightKg??'—'} кг × ${set.reps??'—'}`}
function gymWorkoutDate(w){return w.date?new Date(w.date+'T12:00:00').toLocaleDateString('ru-RU',{day:'numeric',month:'long',year:'numeric'}):'Дата не уточнена'}
function currentGymWorkout(){ensureFitness();return state.fitness.gym.workouts.find(w=>w.status==='active')||null}
function previousGymEntry(exerciseId,excludeId){return state.fitness.gym.workouts.filter(w=>w.id!==excludeId&&w.status!=='active').sort((a,b)=>String(b.date||b.finishedAt).localeCompare(String(a.date||a.finishedAt))).map(w=>(w.exercises||[]).find(e=>e.exerciseId===exerciseId)).find(Boolean)||null}
function gym(){ensureFitness();shell(`<section class="screen fitnessScreen gymScreen"><div class="screenTop"><button id="backGym" class="backBtn">← Фитнес</button><div><small>MYOS · V0.24.8</small><h2>🏋️ Тренажёрный зал</h2></div></div><nav class="gymTabs">${[['today','Тренировка сегодня'],['history','История'],['exercises','Упражнения'],['progress','Прогресс']].map(x=>`<button data-gym-tab="${x[0]}" class="${gymView===x[0]?'active':''}">${x[1]}</button>`).join('')}</nav><div id="gymContent"></div></section>`);document.getElementById('backGym').onclick=fitness;document.querySelectorAll('[data-gym-tab]').forEach(b=>b.onclick=()=>{gymView=b.dataset.gymTab;gym()});renderGymView()}
function renderGymView(){if(gymView==='history')return gymHistory();if(gymView==='exercises')return gymExercises();if(gymView==='progress')return gymProgress();return gymToday()}
function gymToday(){const box=document.getElementById('gymContent'),w=currentGymWorkout();if(!w){box.innerHTML=`<section class="card gymStart"><span>🏋️</span><h2>Тренировка сегодня</h2><p>Записывайте подходы прямо во время занятия. Всё сразу сохраняется локально и отправляется в облако при наличии сети.</p><button id="startGym" class="workPrimary">Начать тренировку</button></section>`;document.getElementById('startGym').onclick=()=>{const now=new Date();state.fitness.gym.workouts.push({id:`gym-${Date.now()}-${MYOS_DEVICE_ID}`,date:isoLocal(now),startedAt:now.toISOString(),finishedAt:null,location:'gym',type:'gym',status:'active',exercises:[]});save();gymToday()};return}box.innerHTML=`<section class="gymLiveHead"><div><span class="kicker">ТРЕНИРОВКА СЕГОДНЯ</span><h2>${gymWorkoutDate(w)}</h2></div><button id="finishGym" class="workSecondary">Завершить тренировку</button></section><section class="gymLiveList">${(w.exercises||[]).map(e=>gymLiveExercise(w,e)).join('')}</section><button id="addGymExercise" class="workPrimary">＋ Добавить упражнение</button>`;document.getElementById('finishGym').onclick=()=>{if(!(w.exercises||[]).length&&!confirm('Завершить тренировку без упражнений?'))return;w.finishedAt=new Date().toISOString();w.status='finished';save();gymView='history';gym()};document.getElementById('addGymExercise').onclick=()=>gymExercisePicker(w.id);bindGymSetForms(w)}
function gymLiveExercise(w,e){const prev=previousGymEntry(e.exerciseId,w.id),last=prev&&prev.sets&&prev.sets.at(-1);return `<article class="card gymExerciseLive"><header><div><b>${escapeHtml(e.name)}</b><small>${escapeHtml(e.category)}</small></div>${last?`<span>Прошлый раз: <b>${gymRecordLabel(prev,last)}</b></span>`:'<span>Первый результат</span>'}</header><div class="gymSets">${(e.sets||[]).map((x,i)=>`<div><i>${i+1}</i><b>${gymRecordLabel(e,x)}</b>${x.note?`<small>${escapeHtml(x.note)}</small>`:''}</div>`).join('')}</div><div class="gymSetForm" data-entry="${e.id}">${e.kind==='cardio'?'<label>Км<input inputmode="decimal" data-distance type="number" min="0" step="0.01"></label><label>Минуты<input inputmode="numeric" data-duration type="number" min="0" step="1"></label>':'<label>Вес, кг<input inputmode="decimal" data-weight type="number" min="0" step="0.5"></label><label>Повторы<input inputmode="numeric" data-reps type="number" min="0" step="1"></label>'}<label class="gymNote">Заметка<input data-note maxlength="120" placeholder="Необязательно"></label>${e.kind==='strength'&&e.sets.length?'<button type="button" data-copy>Копировать вес</button>':''}<button type="button" class="workPrimary" data-add-set>Добавить подход</button></div></article>`}
function bindGymSetForms(w){document.querySelectorAll('.gymSetForm').forEach(form=>{const e=w.exercises.find(x=>x.id===form.dataset.entry);const weight=form.querySelector('[data-weight]');const last=e.sets.at(-1);const copy=form.querySelector('[data-copy]');if(copy)copy.onclick=()=>{weight.value=last&&last.weightKg!=null?last.weightKg:'';weight.focus()};form.querySelector('[data-add-set]').onclick=()=>{let set={id:`gym-set-${Date.now()}-${MYOS_DEVICE_ID}`},note=form.querySelector('[data-note]').value.trim();if(e.kind==='cardio'){const distance=+form.querySelector('[data-distance]').value,duration=+form.querySelector('[data-duration]').value;if(!(distance>0)||!(duration>0))return alert('Укажите дистанцию и время.');set.distanceKm=distance;set.durationSeconds=Math.round(duration*60)}else{const repsInput=form.querySelector('[data-reps]'),kg=+weight.value,reps=+repsInput.value;if(weight.value===''||repsInput.value===''||!(kg>=0)||!(reps>0))return alert('Укажите вес и повторения.');set.weightKg=kg;set.reps=reps}if(note)set.note=note;e.sets.push(set);save();gymToday()}})}
function gymExercisePicker(workoutId){const w=state.fitness.gym.workouts.find(x=>x.id===workoutId);shell(`<section class="screen fitnessScreen"><div class="screenTop"><button id="backPicker" class="backBtn">← Тренировка</button><div><small>MYOS · V0.24.8</small><h2>Добавить упражнение</h2></div></div><input id="gymSearch" class="gymSearch" placeholder="Поиск упражнения"><section id="gymPickList" class="gymPickList"></section><button id="newGymExercise" class="workSecondary">＋ Своё упражнение</button></section>`);const draw=()=>{const q=gymSearch.value.trim().toLowerCase();gymPickList.innerHTML=state.fitness.gym.exercises.filter(x=>x.name.toLowerCase().includes(q)).map(x=>`<button class="card" data-pick="${x.id}"><span>${x.kind==='cardio'?'🏃':'🏋️'}</span><b>${escapeHtml(x.name)}</b><small>${escapeHtml(x.category)}</small></button>`).join('');gymPickList.querySelectorAll('[data-pick]').forEach(b=>b.onclick=()=>addGymExercise(w,b.dataset.pick))};backPicker.onclick=()=>gym();gymSearch.oninput=draw;newGymExercise.onclick=()=>createGymExercise(w);draw()}
function addGymExercise(w,id){const x=state.fitness.gym.exercises.find(e=>e.id===id);if(!x)return;if(!w.exercises.some(e=>e.exerciseId===id))w.exercises.push({id:`gym-entry-${Date.now()}-${MYOS_DEVICE_ID}`,exerciseId:x.id,name:x.name,category:x.category,kind:x.kind,sets:[]});save();gymView='today';gym()}
function createGymExercise(w){const name=prompt('Название упражнения:','');if(!name||!name.trim())return;const category=prompt(`Категория: ${GYM_CATEGORIES.join(', ')}`,'другое');if(!GYM_CATEGORIES.includes(category))return alert('Выберите категорию из списка.');const kind=category==='кардио'?'cardio':'strength',x={id:`gym-ex-${Date.now()}-${MYOS_DEVICE_ID}`,name:name.trim(),category,kind};state.fitness.gym.exercises.push(x);save();addGymExercise(w,x.id)}
function gymHistory(){const box=document.getElementById('gymContent'),dated=state.fitness.gym.workouts.filter(x=>x.status!=='active').sort((a,b)=>String(b.date).localeCompare(String(a.date)));box.innerHTML=`<section class="gymHistory">${dated.map(w=>`<button class="card" data-workout="${w.id}"><div><b>${gymWorkoutDate(w)}</b><small>${(w.exercises||[]).map(e=>e.name).join(' · ')||'Без упражнений'}</small></div><i>›</i></button>`).join('')}</section><div class="sectionTitle"><h2>Ожидают уточнения даты</h2><span>${state.fitness.gym.legacyDrafts.length}</span></div><section class="gymDrafts">${state.fitness.gym.legacyDrafts.filter(x=>!x.resolvedWorkoutId).map(x=>`<article class="card"><b>${escapeHtml(x.name)}</b><small>${escapeHtml(x.note)}</small><p>${x.sets.map(s=>gymRecordLabel(x,s)).join(' · ')}</p><button class="workSecondary" data-date-draft="${x.id}">Уточнить дату</button></article>`).join('')}</section>`;document.querySelectorAll('[data-workout]').forEach(b=>b.onclick=()=>gymWorkoutDetail(b.dataset.workout));document.querySelectorAll('[data-date-draft]').forEach(b=>b.onclick=()=>resolveGymDraftDate(b.dataset.dateDraft))}
function resolveGymDraftDate(id){const draft=state.fitness.gym.legacyDrafts.find(x=>x.id===id);if(!draft)return;const date=prompt('Дата тренировки (ГГГГ-ММ-ДД):','');if(!localDateKey(date)||date!==localDateKey(date))return alert('Укажите дату в формате ГГГГ-ММ-ДД.');const workoutId=`gym-resolved-${draft.id}`;if(!state.fitness.gym.workouts.some(x=>x.id===workoutId))state.fitness.gym.workouts.push({id:workoutId,date,startedAt:null,finishedAt:null,location:'gym',type:'gym',source:'confirmed-legacy',migrated:true,exercises:[{id:`gym-entry-resolved-${draft.id}`,exerciseId:draft.exerciseId,name:draft.name,category:(state.fitness.gym.exercises.find(x=>x.id===draft.exerciseId)||{}).category||'другое',kind:draft.kind,sets:clone(draft.sets),note:draft.note}]});draft.resolvedWorkoutId=workoutId;draft.resolvedDate=date;save();gymHistory()}
function gymWorkoutDetail(id){const w=state.fitness.gym.workouts.find(x=>x.id===id);if(!w)return gym();shell(`<section class="screen fitnessScreen"><div class="screenTop"><button id="backGymHistory" class="backBtn">← История</button><div><small>MYOS · V0.24.8</small><h2>${gymWorkoutDate(w)}</h2></div></div><section class="gymDetail">${w.exercises.map(e=>`<article class="card"><h3>${e.kind==='cardio'?'🏃':e.kind==='circuit'?'🔁':'🏋️'} ${escapeHtml(e.name)}</h3>${e.sets.length?e.sets.map((x,i)=>`<p><b>${i+1}.</b> ${gymRecordLabel(e,x)}${x.calculated?' · вычислено из исходной записи':''}</p>`).join(''):'<p>Подходы в исходной записи не указаны.</p>'}${e.note?`<small>${escapeHtml(e.note)}</small>`:''}</article>`).join('')}</section></section>`);backGymHistory.onclick=()=>{gymView='history';gym()}}
function gymExercises(){const box=document.getElementById('gymContent');box.innerHTML=`<section class="gymLibrary">${GYM_CATEGORIES.map(c=>`<div><h3>${c}</h3>${state.fitness.gym.exercises.filter(x=>x.category===c).map(x=>`<article class="card"><b>${escapeHtml(x.name)}</b><small>${x.kind==='cardio'?'дистанция · время':'вес · повторения'}</small></article>`).join('')||'<small>Пока пусто</small>'}</div>`).join('')}</section>`}
function exerciseResults(id){const rows=[];state.fitness.gym.workouts.filter(w=>w.status!=='active').forEach(w=>{const e=(w.exercises||[]).find(x=>x.exerciseId===id);if(e)(e.sets||[]).forEach(set=>rows.push({date:w.date,entry:e,set}))});return rows.sort((a,b)=>String(a.date).localeCompare(String(b.date)))}
function gymProgress(){const box=document.getElementById('gymContent'),used=state.fitness.gym.exercises.map(x=>[x,exerciseResults(x.id)]).filter(x=>x[1].length);box.innerHTML=`<section class="gymProgressList">${used.map(([x,r])=>{const last=r.at(-1),prev=r.at(-2),bestKg=Math.max(...r.map(y=>Number(y.set.weightKg)||0)),bestReps=Math.max(...r.map(y=>Number(y.set.reps)||0));let delta='Первый результат';if(prev)delta=x.kind==='cardio'?`Дистанция ${last.set.distanceKm>=prev.set.distanceKm?'↑':'↓'} · время ${last.set.durationSeconds>=prev.set.durationSeconds?'↑':'↓'}`:`Вес ${last.set.weightKg>=prev.set.weightKg?'↑':'↓'} · повторы ${last.set.reps>=prev.set.reps?'↑':'↓'}`;return `<article class="card gymProgressCard"><header><b>${escapeHtml(x.name)}</b><span>${delta}</span></header><p>Последний: <b>${gymRecordLabel(last.entry,last.set)}</b></p>${x.kind==='cardio'?`<p>Темп: <b>${last.set.distanceKm?(last.set.durationSeconds/60/last.set.distanceKm).toFixed(1):'—'} мин/км</b></p>`:`<p>Лучший вес: <b>${bestKg} кг</b> · повторы: <b>${bestReps}</b></p>`}<details><summary>Вся история</summary>${r.map(y=>`<div>${y.date||'Без даты'} → ${gymRecordLabel(y.entry,y.set)}</div>`).join('')}</details></article>`}).join('')||'<article class="card emptyFood"><b>Прогресс появится после подходов</b></article>'}</section>`}
function fitnessHistory(){ensureFitness();const kegel=state.fitness.kegel.history.map(x=>({...x,historyType:'kegel'})),gymItems=state.fitness.gym.workouts.filter(x=>x.status!=='active').map(x=>({...x,historyType:'gym'})),items=[...kegel,...gymItems].sort((a,b)=>String(b.completedAt||b.finishedAt||b.date).localeCompare(String(a.completedAt||a.finishedAt||a.date)));shell(`<section class="screen fitnessScreen"><div class="screenTop"><button id="backAllHistory" class="backBtn">← Фитнес</button><div><small>MYOS · V0.24.8</small><h2>📊 История тренировок</h2></div></div><p class="sectionLead">Единая история: Кегель, силовые, домашние и будущие тренировки.</p><section class="allFitnessHistory">${items.map(x=>{const date=x.date?new Date(x.date+'T12:00:00').toLocaleDateString('ru-RU',{day:'numeric',month:'long',year:'numeric'}):'Дата не указана';if(x.historyType==='gym')return `<button class="card" data-all-gym="${x.id}"><span>🏋️</span><div><b>${date} · Силовая</b><small>${(x.exercises||[]).map(e=>escapeHtml(e.name)).join(' · ')||'Упражнения не указаны'}</small></div><i>›</i></button>`;return `<button class="card" data-all-kegel="${x.id}"><span>🧘</span><div><b>${date} · ${x.completed===false?'частично':'выполнена'}</b><small>${x.workoutNumber?`Тренировка №${x.workoutNumber} · `:''}${escapeHtml(x.levelName||'Legacy Kegel')}${Number.isFinite(+x.duration)?` · ${formatTimer(x.duration)}`:''}${x.repetitions?` · ${x.repetitions} повторов`:''}</small></div><i>›</i></button>`}).join('')||'<article class="card emptyFood"><b>История пока пуста</b></article>'}</section></section>`);document.getElementById('backAllHistory').onclick=fitness;document.querySelectorAll('[data-all-kegel]').forEach(b=>b.onclick=()=>kegelHistoryDetail(b.dataset.allKegel));document.querySelectorAll('[data-all-gym]').forEach(b=>b.onclick=()=>gymWorkoutDetail(b.dataset.allGym))}
function kegelHistoryDetail(id){const x=state.fitness.kegel.history.find(h=>h.id===id);if(!x)return fitness();const date=new Date((x.date||'')+'T12:00:00').toLocaleDateString('ru-RU');shell(`<section class="screen fitnessScreen"><div class="screenTop"><button id="backHistory" class="backBtn">← История</button><div><small>MYOS · V0.24.8</small><h2>🧘 ${date}</h2></div></div><section class="card historyDetail"><span class="kicker">${x.legacy?'ИМПОРТИРОВАНО ИЗ CHATGPT':'ТРЕНИРОВКА ВЫПОЛНЕНА'}</span><h2>${escapeHtml(x.levelName||'Кегель · выполнено')}</h2>${x.note?`<p>${escapeHtml(x.note)}</p>`:''}<dl><div><dt>Время</dt><dd>${x.completedAt?new Date(x.completedAt).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'}):'Неизвестно'}</dd></div><div><dt>Длительность</dt><dd>${Number.isFinite(+x.duration)?formatTimer(x.duration):'Неизвестно'}</dd></div><div><dt>Упражнения</dt><dd>${x.exerciseNames?x.exerciseNames.map(escapeHtml).join(', '):'Неизвестно'}</dd></div><div><dt>Циклы / повторы</dt><dd>${x.cycles??'Неизвестно'}</dd></div>${x.feedback?`<div><dt>Отзыв</dt><dd>${escapeHtml(x.feedback)}</dd></div>`:''}</dl></section></section>`);document.getElementById('backHistory').onclick=fitness}
function startKegel(){clearInterval(workoutInterval);const plan=kegelPlan();workoutRuntime={plan,index:0,remaining:plan.phases[0].seconds,running:false,endAt:null,startedAt:null,finished:false,completedPhaseIndexes:[],skippedPhaseIndexes:[]};renderWorkout()}
...
function finishWorkoutPhase(skipped=false){const w=workoutRuntime;if(!w)return;if(skipped)w.skippedPhaseIndexes.push(w.index);else w.completedPhaseIndexes.push(w.index)}
function workoutTick(now=Date.now()){
 ...
 while(w.running&&now>=w.endAt){finishWorkoutPhase(false);w.index++;phaseSignal();if(w.index>=w.plan.phases.length){w.running=false;w.finished=true;clearInterval(workoutInterval);renderWorkoutComplete();return}w.endAt+=w.plan.phases[w.index].seconds*1000;w.remaining=Math.max(0,(w.endAt-now)/1000)}renderWorkout();
}
function workoutProgressPercent(w){const done=w.completedPhaseIndexes.length+w.skippedPhaseIndexes.length,current=w.plan.phases[w.index],fraction=current?1-w.remaining/current.seconds:0;return Math.min(100,Math.round((done+fraction)/w.plan.phases.length*100))}
function updateWorkoutDisplay(){const w=workoutRuntime,p=w&&w.plan.phases[w.index],timer=document.getElementById('workoutTimer');if(!p||!timer)return;timer.textContent=formatTimer(w.remaining);const percent=workoutProgressPercent(w),progress=document.getElementById('workoutProgress');if(progress)progress.style.width=`${percent}%`;const overall=document.getElementById('workoutOverall');if(overall)overall.textContent=`Общий прогресс ${percent}%`}
function exercisePreview(x){return `<section class="exercisePreview"><span class="kicker">СЛЕДУЮЩЕЕ</span><h2>Упражнение ${x.index} · ${escapeHtml(x.name)}</h2><p>${escapeHtml(x.goal)}</p><dl><div><dt>Повторов / циклов</dt><dd>${x.reps}</dd></div>${x.work?`<div><dt>Сжатие</dt><dd>${x.work} сек</dd></div>`:''}${x.relax?`<div><dt>Расслабление</dt><dd>${x.relax} сек</dd></div>`:''}<div><dt>Интенсивность</dt><dd>${escapeHtml(x.intensity||'без силового усилия')}</dd></div></dl><small>${escapeHtml(x.technique)}</small></section>`}
function renderWorkout(){const w=workoutRuntime,p=w.plan.phases[w.index],isRest=p.phase==='ОТДЫХ',percent=workoutProgressPercent(w);shell(`<section class="workoutScreen ${isRest?'phase-rest':'phase-work'}"><button id="exitWorkout" class="workoutExit">✕ Завершить</button><div class="workoutProgress"><i id="workoutProgress" style="width:${percent}%"></i></div><div class="workoutMeta"><span>Упражнение ${p.nextExercise?p.nextExercise.index:p.exerciseIndex} из ${p.exerciseTotal}</span><b id="workoutOverall">Общий прогресс ${percent}%</b></div>${isRest?`<span class="kicker">ВОССТАНОВЛЕНИЕ</span><h1>ОТДЫХ</h1>`:`<span class="kicker">Упражнение ${p.exerciseIndex} · ${escapeHtml(p.exercise)}</span><h1>${escapeHtml(p.action)}</h1><div class="workoutIntensity"><small>ИНТЕНСИВНОСТЬ</small><b>${escapeHtml(p.intensityName||'Без силового усилия')}</b>${p.intensity?`<span>${escapeHtml(p.intensity)}</span>`:''}</div>`}<strong id="workoutTimer">${formatTimer(w.remaining)}</strong>${p.repeat?`<b id="workoutCounter">${p.phase==='REVERSE KEGEL'?'Цикл':'Повтор'} ${p.repeat} из ${p.total}</b>`:''}<p>${escapeHtml(p.instruction)}</p>${isRest&&p.nextExercise?exercisePreview(p.nextExercise):''}<div class="workoutControls"><button id="toggleWorkout" class="workPrimary">${w.running?'⏸ Пауза':w.startedAt?'▶️ Продолжить':'▶️ Старт'}</button>${isRest?'<button id="skipWorkout" class="workSecondary">Начать раньше</button>':'<button id="skipWorkout" class="workSecondary">⏭ Пропустить фазу</button>'}</div></section>`);updateWorkoutDisplay();document.getElementById('toggleWorkout').onclick=toggleWorkout;document.getElementById('skipWorkout').onclick=()=>skipWorkoutPhase();document.getElementById('exitWorkout').onclick=()=>{if(confirm('Завершить тренировку без записи в историю?')){clearInterval(workoutInterval);workoutRuntime=null;fitness()}}}
function skipWorkoutPhase(){const w=workoutRuntime;finishWorkoutPhase(true);w.index++;if(w.index>=w.plan.phases.length){w.running=false;w.finished=true;clearInterval(workoutInterval);return renderWorkoutComplete()}w.remaining=w.plan.phases[w.index].seconds;if(w.running)w.endAt=Date.now()+w.remaining*1000;renderWorkout()}
...
function renderWorkoutComplete(){const w=workoutRuntime,completed=w.completedPhaseIndexes.length,all=w.plan.phases.length,status=completed===all?'Выполнена полностью':'Выполнена частично';shell(`<section class="workoutComplete"><span>✓</span><h1>Тренировка завершена</h1><p>${status} · ${completed} из ${all} фаз</p><section class="card"><div><small>Фактическое время</small><b>${formatTimer(w.completedPhaseIndexes.reduce((n,i)=>n+w.plan.phases[i].seconds,0))}</b></div><div><small>Упражнения</small><b>${w.plan.exercises.length}</b></div><div><small>Тренировка</small><b>№${w.plan.number}</b></div></section><label class="card">Оценка или заметка (необязательно)<textarea id="kegelFeedback" placeholder="Что получилось, что было трудно?"></textarea></label><button id="saveWorkout" class="workPrimary">✓ Сохранить в историю</button><button id="repeatWorkout" class="workSecondary">↻ Повторить</button></section>`);document.getElementById('saveWorkout').onclick=saveKegelWorkout;document.getElementById('repeatWorkout').onclick=startKegel}
function saveKegelWorkout(){const w=workoutRuntime;if(!w||w.saved)return fitness();w.saved=true;const now=new Date(),completedPhases=w.completedPhaseIndexes.map(i=>w.plan.phases[i]),duration=completedPhases.reduce((n,p)=>n+p.seconds,0),feedback=(document.getElementById('kegelFeedback')||{}).value||'',id=`kegel-${MYOS_DEVICE_ID}-${w.startedAt||now.getTime()}`;if(!state.fitness.kegel.history.some(x=>x.id===id))state.fitness.kegel.history.push({id,date:isoLocal(now),workoutType:'kegel',type:'kegel',completed:w.completedPhaseIndexes.length===w.plan.phases.length,completionStatus:w.completedPhaseIndexes.length===w.plan.phases.length?'completed':'partial',source:'MyOS Fitness',workoutNumber:w.plan.number,level:w.plan.level,levelName:w.plan.name,duration,exercises:w.plan.exercises.length,exerciseNames:w.plan.exercises.map(x=>x.name),exercisePlan:clone(w.plan.exercises),intervals:completedPhases.map(p=>({exercise:p.exercise,exerciseIndex:p.exerciseIndex,phase:p.phase,action:p.action,seconds:p.seconds,repeat:p.repeat||null,total:p.total||null,intensity:p.intensity||null})),completedPhaseIndexes:clone(w.completedPhaseIndexes),skippedPhaseIndexes:clone(w.skippedPhaseIndexes),cycles:completedPhases.reduce((n,p)=>n+(p.cycles||0),0),repetitions:completedPhases.filter(p=>p.phase==='СЖАТЬ'||p.phase==='REVERSE KEGEL').length,feedback:feedback.trim(),startedAt:w.startedAt?new Date(w.startedAt).toISOString():null,completedAt:now.toISOString()});state.fitness.kegel.programLevel=suggestedKegelLevel();state.fitness.kegel.selectedLevel=state.fitness.kegel.programLevel;save();workoutRuntime=null;fitness()}
...
function nutrition(){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backMe" class="backBtn">← Я</button><div><small>MYOS · V0.24.8</small><h2>🍽 Питание</h2></div></div>
...
function mealAddChoice(date){
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backNutrition" class="backBtn">← Питание</button><div><small>MYOS · V0.24.8</small><h2>Добавить еду</h2></div></div>
...
function aiMealForm(date,mode){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backAddMeal" class="backBtn">← Добавить</button><div><small>MYOS · V0.24.8</small><h2>${photo?'📷 Еда по фото':'💬 Еда по описанию'}</h2></div></div>
