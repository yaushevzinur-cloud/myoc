...
function ensureFitness(){
 ...
 importKegelLegacyRecords(KNOWN_KEGEL_LEGACY,k,false);
 const completedLevels=k.history.filter(x=>x&&x.completed&&Number.isFinite(+x.level)).map(x=>+x.level);
 if(!Number.isFinite(+k.currentLevel))k.currentLevel=Math.max(1,+k.selectedLevel||0,...completedLevels);
 if(!Number.isFinite(+k.selectedLevel))k.selectedLevel=k.currentLevel;
 if(!k.legacyProgramImported){k.selectedLevel=Math.max(+k.selectedLevel||0,k.currentLevel);k.legacyProgramImported=true}
}
const KNOWN_KEGEL_LEGACY=[
 {date:"2026-09-08",legacyNote:"Первая достоверно известная выполненная тренировка программы."},
 {date:"2026-09-09",legacyNote:"Следующая последовательная тренировка программы выполнена."},
 {date:"2026-09-21",legacyNote:"Контролируемое сокращение и полноценное расслабление уже различаются. Короткие сокращения и постепенные удержания освоены; reverse Kegel пока ощущается слабо и требует мягкой отработки."}
];
function kegelLegacyId(date,type="kegel"){return `kegel-legacy-${date}-${String(type).toLowerCase().replace(/[^a-z0-9а-я]+/gi,"-")}`}
// Date + workout type is the import key. A later ChatGPT journal import enriches
// the same record rather than adding a second session; omitted legacy details
// remain omitted instead of being replaced with invented zeroes.
function importKegelLegacyRecords(records,kegel=state.fitness&&state.fitness.kegel,overwrite=true){
 if(!kegel||!Array.isArray(kegel.history)||!Array.isArray(records))return 0;
 let changed=0;
 records.forEach(input=>{
  if(!input||!/^\d{4}-\d{2}-\d{2}$/.test(input.date||""))return;
  const workoutType=input.workoutType||"kegel",id=kegelLegacyId(input.date,workoutType);
  let record=kegel.history.find(x=>x&&((x.id===id)||(x.imported&&x.date===input.date&&(x.workoutType||"kegel")===workoutType)));
  if(!record){record={id,date:input.date,workoutType,status:"completed",completed:true,source:"ChatGPT legacy",legacy:true,imported:true};kegel.history.push(record);changed++}
  Object.entries(input).forEach(([key,value])=>{if(value!==undefined&&value!==null&&value!==""&&(overwrite||record[key]===undefined||record[key]===null||record[key]==="")&&JSON.stringify(record[key])!==JSON.stringify(value)){record[key]=clone(value);changed++}});
  record.id=id;record.date=input.date;record.workoutType=workoutType;record.status="completed";record.completed=true;record.source="ChatGPT legacy";record.legacy=true;record.imported=true;
 });
 return changed;
}
ensureFitness();
...
function suggestedKegelLevel(){ensureFitness();const completed=state.fitness.kegel.history.filter(x=>x.completed&&Number.isFinite(+x.level)).map(x=>+x.level);return Math.min(KEGEL_LEVELS.length-1,Math.max(+state.fitness.kegel.currentLevel||0,...completed))}
...
function kegelDateLabel(x,options={day:'numeric',month:'long'}){return new Date(`${x.date}T12:00:00`).toLocaleDateString('ru-RU',options)}
function kegelRecordSummary(x){if(x.legacy||x.imported)return `Импортировано из ChatGPT · ${escapeHtml(x.note||x.legacyNote||'точные параметры пока не импортированы')}`;return `${escapeHtml(x.levelName||'Тренировка Кегеля')} · ${Math.ceil((+x.duration||0)/60)} мин · ${+x.cycles||0} циклов`}
function kegelTodayCard(){const done=kegelToday(),plan=kegelPlan();return `<button id="todayKegel" class="todayKegel card ${done?'done':''}"><span>${done?'✓':'🧘'}</span><span><b>${done?'Кегель выполнен сегодня':'Кегель · сегодня'}</b><small>${done?kegelRecordSummary(done):`~ ${Math.ceil(plan.duration/60)} мин`}</small></span><i>${done?'История':'Начать'}</i></button>`}
function today(){
...
function fitness(){
 ...
 shell(`<section class="screen fitnessScreen"><div class="screenTop"><button id="backFitness" class="backBtn">← Я</button><div><small>MYOS · V0.24.3</small><h2>🏋️ Фитнес / Тело</h2></div></div><p class="sectionLead">Центр тренировок и прогресса тела.</p><section class="fitnessDirections"><article class="card"><span>🏋️</span><b>Силовые тренировки</b><small>Существующая программа сохраняется</small></article><article class="card"><span>🏠</span><b>Домашние тренировки</b><small>Интервальные программы — далее</small></article><button id="openKegel" class="card active"><span>🧘</span><b>Кегель / тазовое дно</b><small>Ежедневная спокойная практика</small></button><article class="card"><span>📊</span><b>История тренировок</b><small>${completed} тренировок Кегеля</small></article></section><section class="card kegelHero"><span class="kicker">КЕГЕЛЬ — ТРЕНИРОВКА СЕГОДНЯ</span><h2>${done?'✓ Выполнена сегодня':`Следующая тренировка · ${escapeHtml(plan.name)}`}</h2><p>${done?'Запись уже сохранена в истории':`Примерно ${Math.ceil(plan.duration/60)} мин · ${plan.cycles} циклов · акцент на полном расслаблении и reverse Kegel`}</p><label>Уровень<select id="kegelLevel">${KEGEL_LEVELS.map((x,i)=>`<option value="${i}" ${i===plan.level?'selected':''}>${i+1}. ${escapeHtml(x.name)}${i===suggestedKegelLevel()?' · текущий':''}</option>`).join('')}</select></label><button class="workPrimary" id="startKegel">${done?'Повторить тренировку':'Начать тренировку'}</button><button class="workSecondary" id="importKegel">Дополнить историю из ChatGPT</button></section><section class="kegelStats"><div class="card"><small>ТЕКУЩАЯ СЕРИЯ</small><b>${kegelStreak()} дней</b></div><div class="card"><small>ВСЕГО</small><b>${completed}</b></div></section><div class="sectionTitle"><h2>История Кегеля</h2><span>по датам</span></div><section class="kegelHistory">${history.length?history.map(x=>`<button class="card" data-kegel-history="${x.id}"><b>${x.date?kegelDateLabel(x):'Перенесённый прогресс'}</b><small>${kegelRecordSummary(x)}</small><i>›</i></button>`).join(''):`<article class="card emptyFood"><b>История пока пуста</b><small>Завершённые тренировки появятся здесь и синхронизируются между устройствами.</small></article>`}</section></section>`);
 document.getElementById('backFitness').onclick=()=>render('me');document.getElementById('openKegel').onclick=()=>document.querySelector('.kegelHero').scrollIntoView({behavior:'smooth'});document.getElementById('kegelLevel').onchange=e=>{state.fitness.kegel.selectedLevel=+e.target.value;state.fitness.kegel.currentLevel=+e.target.value;save();fitness()};document.getElementById('startKegel').onclick=()=>startKegel();document.getElementById('importKegel').onclick=()=>{const value=prompt('Вставьте JSON-массив записей старого журнала. Обязательное поле каждой записи — date (ГГГГ-ММ-ДД). Повторная дата обновит существующую импортированную тренировку, а не создаст дубль.','');if(value===null||!value.trim())return;try{const records=JSON.parse(value);if(!Array.isArray(records))throw new Error();const changed=importKegelLegacyRecords(records);if(!changed)return alert('Новых точных данных для дополнения не найдено.');save();fitness()}catch(e){alert('Не удалось прочитать данные. Нужен JSON-массив с полем date.')}};document.querySelectorAll('[data-kegel-history]').forEach(b=>b.onclick=()=>kegelHistoryDetail(b.dataset.kegelHistory));
}
function kegelHistoryDetail(id){const x=state.fitness.kegel.history.find(h=>h.id===id);if(!x)return fitness();const known=v=>v!==undefined&&v!==null&&v!=='';shell(`<section class="screen fitnessScreen"><div class="screenTop"><button id="backHistory" class="backBtn">← История</button><div><small>MYOS · V0.24.3</small><h2>🧘 ${x.date?kegelDateLabel(x,{day:'numeric',month:'long',year:'numeric'}):'Перенесённый прогресс'}</h2></div></div><section class="card historyDetail"><span class="kicker">${x.legacy||x.imported?'ИМПОРТИРОВАНО ИЗ CHATGPT':'ТРЕНИРОВКА ВЫПОЛНЕНА'}</span><h2>${escapeHtml(x.levelName||'Кегель · выполнено')}</h2>${x.note||x.legacyNote?`<p>${escapeHtml(x.note||x.legacyNote)}</p>`:''}<dl>${known(x.completedAt)?`<div><dt>Время</dt><dd>${new Date(x.completedAt).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'})}</dd></div>`:''}${known(x.duration)?`<div><dt>Длительность</dt><dd>${formatTimer(x.duration)}</dd></div>`:''}${known(x.exercises)?`<div><dt>Упражнения</dt><dd>${Array.isArray(x.exerciseNames)?x.exerciseNames.map(escapeHtml).join(', '):x.exercises}</dd></div>`:''}${known(x.cycles)?`<div><dt>Циклы / повторы</dt><dd>${x.cycles}</dd></div>`:''}${x.feedback?`<div><dt>Отзыв</dt><dd>${escapeHtml(x.feedback)}</dd></div>`:''}</dl>${x.legacy||x.imported?'<small>Точные интервалы и повторы не были известны при переносе. Запись можно безопасно дополнить позднее.</small>':''}</section></section>`);document.getElementById('backHistory').onclick=fitness}
function startKegel(){clearInterval(workoutInterval);const plan=kegelPlan();workoutRuntime={plan,index:0,remaining:plan.phases[0].seconds,running:false,endAt:null,startedAt:null,finished:false,actualSeconds:{},skipped:{}};renderWorkout()}
...
function workoutTick(now=Date.now()){
 const w=workoutRuntime;if(!w||!w.running||w.finished)return;w.actualSeconds=w.actualSeconds||{};w.skipped=w.skipped||{};if(now<w.endAt){w.remaining=(w.endAt-now)/1000;updateWorkoutDisplay();return}
 while(w.running&&now>=w.endAt){if(!w.skipped[w.index])w.actualSeconds[w.index]=w.plan.phases[w.index].seconds;w.index++;phaseSignal();if(w.index>=w.plan.phases.length){w.running=false;w.finished=true;clearInterval(workoutInterval);renderWorkoutComplete();return}w.endAt+=w.plan.phases[w.index].seconds*1000;w.remaining=Math.max(0,(w.endAt-now)/1000)}updateWorkoutDisplay();
}
...
function renderWorkoutComplete(){const w=workoutRuntime;const exercises=new Set(w.plan.phases.map(p=>p.exercise)).size,actual=Math.round(w.plan.phases.reduce((sum,p,i)=>sum+(w.actualSeconds[i]??(w.skipped[i]?0:p.seconds)),0));shell(`<section class="workoutComplete"><span>✓</span><h1>Тренировка завершена</h1><p>${new Date().toLocaleDateString('ru-RU')}</p><section class="card"><div><small>Длительность</small><b>${formatTimer(actual)}</b></div><div><small>Упражнения</small><b>${exercises}</b></div><div><small>Циклы / повторы</small><b>${w.plan.cycles}</b></div></section><label class="card">Как ощущалась тренировка?<textarea id="kegelFeedback" placeholder="Необязательно: расслабление, reverse Kegel, комфорт нагрузки"></textarea></label><button id="saveWorkout" class="workPrimary">✓ Завершить</button><button id="repeatWorkout" class="workSecondary">↻ Повторить</button></section>`);document.getElementById('saveWorkout').onclick=saveKegelWorkout;document.getElementById('repeatWorkout').onclick=startKegel}
function saveKegelWorkout(){const w=workoutRuntime;if(!w||w.saved)return fitness();w.saved=true;const now=new Date(),exercises=[...new Set(w.plan.phases.map(p=>p.exercise))],intervals=w.plan.phases.map((p,i)=>({exercise:p.exercise,phase:p.phase,plannedSeconds:p.seconds,actualSeconds:w.actualSeconds[i]??(w.skipped[i]?0:p.seconds),repetitions:w.skipped[i]?0:(p.cycles||0),skipped:!!w.skipped[i]})),duration=Math.round(intervals.reduce((sum,x)=>sum+x.actualSeconds,0)),feedback=(document.getElementById('kegelFeedback')||{}).value||'';state.fitness.kegel.history.push({id:`kegel-${Date.now()}-${MYOS_DEVICE_ID}`,date:isoLocal(now),time:now.toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'}),workoutType:'kegel',program:'MyOS Kegel',status:'completed',completed:true,level:w.plan.level,levelName:w.plan.name,duration,exercises:exercises.length,exerciseNames:exercises,intervals,cycles:intervals.reduce((sum,x)=>sum+x.repetitions,0),completedAt:now.toISOString(),feedback:feedback.trim()});state.fitness.kegel.currentLevel=w.plan.level;save();workoutRuntime=null;fitness()}
