...
function header(title,sub=""){return `<div class="top"><div><span class="eyebrow">MYOS · V0.23.1</span><h1>${title}</h1><div class="muted">${sub}</div><span id="cloudSync" class="cloudSync">${window.MYOS_SYNC_STATUS||"☁ Проверка…"}</span></div><button class="mode" id="mode">${state.mode==="Вахта"?"⛺":"🏠"} ${state.mode}</button></div>`}
...
function contractDetail(id){
 ...
 shell(`<section class="screen workSectionScreen"><div class="screenTop"><button id="backContracts" class="backBtn">← Договоры</button><div><small>MYOS · V0.23.1</small><h2>№${c.number}</h2></div></div>
...
function nutrition(){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backMe" class="backBtn">← Я</button><div><small>MYOS · V0.23.1</small><h2>🍽 Питание</h2></div></div>
...
function mealAddChoice(date){
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backNutrition" class="backBtn">← Питание</button><div><small>MYOS · V0.23.1</small><h2>Добавить еду</h2></div></div>
...
function aiMealForm(date,mode){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backAddMeal" class="backBtn">← Добавить</button><div><small>MYOS · V0.23.1</small><h2>${photo?'📷 Еда по фото':'💬 Еда по описанию'}</h2></div></div>
...
function nutritionImport(date){
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backNutrition" class="backBtn">← Питание</button><div><small>MYOS · V0.23.1</small><h2>📥 Результат из ChatGPT</h2></div></div><section class="card importFoodCard"><b>Вставь MYOS-код из чата</b><small>Можно вставить чистый JSON или строку, начинающуюся с MYOS:. Записи добавятся к выбранному дню.</small><textarea id="foodImportText" placeholder='MYOS:{"meals":[{"type":"Завтрак","name":"...","kcal":500,"protein":30,"fat":15,"carbs":60}]}'></textarea><button class="workSecondary" id="pasteFood">📋 Вставить из буфера</button><div id="importPreview" class="importPreview"></div></section><button class="workPrimary" id="doFoodImport">Импортировать</button></section>`);
 document.getElementById('backNutrition').onclick=()=>nutrition();
 ...
 document.getElementById('doFoodImport').onclick=()=>{let raw=document.getElementById('foodImportText').value.trim();const pos=raw.indexOf('MYOS:');if(pos>=0)raw=raw.slice(pos+5).trim();if(raw.startsWith('```'))raw=raw.replace(/^```(?:json)?\s*/i,'').replace(/```\s*$/,'').trim();let obj;try{obj=JSON.parse(raw)}catch(e){return alert('Не удалось прочитать MYOS-код. Скопируйте блок из ChatGPT целиком.')}const meals=normalizeImportedMeals(obj);if(!meals.length)return alert('В данных не найдено приёмов пищи.');nutritionDay(date).meals.push(...meals);save();alert(`Добавлено записей: ${meals.length}`);nutrition()}
}
function nutritionTargets(){ensureNutrition();const g=state.nutrition.targets;shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backNutrition" class="backBtn">← Питание</button><div><small>MYOS · V0.23.1</small><h2>⚙️ Дневные цели</h2></div></div><section class="card foodForm"><div class="foodMacroInputs"><label>Ккал<input id="tgK" type="number" value="${g.kcal}"></label><label>Белки, г<input id="tgP" type="number" value="${g.protein}"></label><label>Жиры, г<input id="tgF" type="number" value="${g.fat}"></label><label>Углеводы, г<input id="tgC" type="number" value="${g.carbs}"></label></div></section><button class="workPrimary" id="saveTargets">Сохранить цели</button></section>`);document.getElementById('backNutrition').onclick=()=>nutrition();document.getElementById('saveTargets').onclick=()=>{state.nutrition.targets={kcal:+tgK.value||0,protein:+tgP.value||0,fat:+tgF.value||0,carbs:+tgC.value||0};save();nutrition()}}
