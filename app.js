...
// Keep this pre-migration snapshot in memory and re-merge it until V0.24.2's
...
function ensureFitness(){
 ...
 const k=state.fitness.kegel;if(!Array.isArray(k.history))k.history=[];if(!Number.isFinite(+k.selectedLevel))k.selectedLevel=0;
}
ensureFitness();
const KEGEL_LEVELS=[
...
function suggestedKegelLevel(){ensureFitness();return Math.min(KEGEL_LEVELS.length-1,Math.floor(state.fitness.kegel.history.length/7))}
