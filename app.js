...
function header(title,sub=""){return `<div class="top"><div><span class="eyebrow">MYOS · V0.24.4</span><h1>${title}</h1><div class="muted">${sub}</div><span id="cloudSync" class="cloudSync">${window.MYOS_SYNC_STATUS||"☁ Проверка…"}</span></div><button class="mode" id="mode">${state.mode==="Вахта"?"⛺":"🏠"} ${state.mode}</button></div>`}
...
function suggestedKegelLevel(){ensureFitness();const levels=state.fitness.kegel.history.map(x=>Number(x.level)).filter(Number.isFinite);return levels.length?Math.min(KEGEL_LEVELS.length-1,Math.max(...levels)):Math.min(1,Math.floor(kegelCompletedCount()/3))}
...
function contractDetail(id){
 ...
 shell(`<section class="screen workSectionScreen"><div class="screenTop"><button id="backContracts" class="backBtn">← Договоры</button><div><small>MYOS · V0.24.4</small><h2>№${c.number}</h2></div></div>
...
function renderWorkoutComplete(){const w=workoutRuntime;const exercises=new Set(w.plan.phases.map(p=>p.exercise)).size;shell(`<section class="workoutComplete"><span>✓</span><h1>Тренировка завершена</h1><p>${new Date().toLocaleDateString('ru-RU')}</p><section class="card"><div><small>Длительность</small><b>${formatTimer(w.plan.duration)}</b></div><div><small>Упражнения</small><b>${exercises}</b></div><div><small>Циклы / повторы</small><b>${w.plan.cycles}</b></div></section><label class="card">Отзыв о тренировке (необязательно)<textarea id="kegelFeedback" placeholder="Что получилось, что было трудно?"></textarea></label><button id="saveWorkout" class="workPrimary">✓ Завершить</button><button id="repeatWorkout" class="workSecondary">↻ Повторить</button></section>`);document.getElementById('saveWorkout').onclick=saveKegelWorkout;document.getElementById('repeatWorkout').onclick=startKegel}
function saveKegelWorkout(){const w=workoutRuntime;if(!w||w.saved)return fitness();w.saved=true;const now=new Date(),exercises=[...new Set(w.plan.phases.map(p=>p.exercise))],feedback=(document.getElementById('kegelFeedback')||{}).value||'';state.fitness.kegel.history.push({id:`kegel-${Date.now()}-${MYOS_DEVICE_ID}`,date:isoLocal(now),workoutType:'kegel',completed:true,source:'MyOS Fitness',level:w.plan.level,levelName:w.plan.name,duration:w.plan.duration,exercises:exercises.length,exerciseNames:exercises,intervals:w.plan.phases.map(p=>({exercise:p.exercise,phase:p.phase,seconds:p.seconds,repeat:p.repeat||null,total:p.total||null})),cycles:w.plan.cycles,feedback:feedback.trim(),startedAt:w.startedAt?new Date(w.startedAt).toISOString():null,completedAt:now.toISOString()});state.fitness.kegel.programLevel=w.plan.level;save();workoutRuntime=null;fitness()}
...
function nutrition(){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backMe" class="backBtn">← Я</button><div><small>MYOS · V0.24.4</small><h2>🍽 Питание</h2></div></div>
...
function mealAddChoice(date){
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backNutrition" class="backBtn">← Питание</button><div><small>MYOS · V0.24.4</small><h2>Добавить еду</h2></div></div>
...
function aiMealForm(date,mode){
 ...
 shell(`<section class="screen nutritionScreen"><div class="screenTop"><button id="backAddMeal" class="backBtn">← Добавить</button><div><small>MYOS · V0.24.4</small><h2>${photo?'📷 Еда по фото':'💬 Еда по описанию'}</h2></div></div>
